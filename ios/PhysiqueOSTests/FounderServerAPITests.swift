...
final class ActiveGoalCurrentStateTests: XCTestCase {
    ...
    func testFormatsNumbersAndDatesOnly() throws {
        ...
        XCTAssertEqual(ActiveGoalFormat.progressHeadline(progress), "Goal progress")
        XCTAssertEqual(ActiveGoalFormat.remainingLabel(progress), "4.2 lb to go")
        for text in [ActiveGoalFormat.progressHeadline(progress), ActiveGoalFormat.remainingLabel(progress) ?? ""] {
