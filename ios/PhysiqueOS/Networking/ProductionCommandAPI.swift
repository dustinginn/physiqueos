...
struct ProductionCommandOutcome<Result: Decodable & Sendable>: Decodable, Sendable {
    ...
    /// automatically drives one step of the same durable Evidence Review
    /// confirmation orchestrator the web app uses immediately after those
    /// two commands, and merges its result in here. `state` is never
...
/// `"processing"` means at least one more step remains; a durable outbox
/// worker keeps advancing it with no additional Native call required —
