...
describe("ProductionPhase7BInfrastructurePreflightInspector", () => {
  it("accepts only a fresh exact backup/DNS/edge/deployment/worker evidence join", async () => {
    ...
    await expect(inspector.inspect({ input: input() })).resolves.toMatchObject({ ready: true, categories: { backups: true, routingZone: true, workerControl: true } });
  });
  it.each([
    ...
    ["foreign backup target", (items) => { items.backups.inspect = async () => ({ ...(await evidence().backups.inspect()), postgresDatabaseName: "physiqueos_production" }); }],
    ...
    const result = await createProductionPhase7BInfrastructurePreflightInspector({ configuration: config(), inspectors: items, now: () => NOW }).inspect({ input: input() });
...
function evidence() {
  ...
  const values = {
    backups: { ...common, windowsEncryptedRestoreVerified: true, windowsIndependentReplicaVerified: true, postgresManagedBackupCurrent: true, postgresIsolatedRestoreVerified: true, spacesIndependentRestoreVerified: true, windowsBackupRef: "backup:windows:one", postgresBackupRef: "backup:postgres:one", spacesBackupRef: "backup:spaces:one", windowsHostId: "phase7b-isolated-windows-restore-1", postgresDatabaseClusterId: "isolated-cluster", postgresDatabaseName: "physiqueos_phase5_restore_provider_phase7b", spacesBucket: "physiqueos-phase7b-isolated-exercise-1", spacesPrefix: "private/phase5-synthetic-user/" },
...
function input() { return { providerWorkerId: "worker-1", windowsHostId: "phase7b-isolated-windows-restore-1", target: { databaseClusterId: "isolated-cluster", databaseName: "physiqueos_phase5_restore_provider_phase7b", spacesBucket: "physiqueos-phase7b-isolated-exercise-1", spacesPrefix: "private/phase5-synthetic-user/" } }; }
