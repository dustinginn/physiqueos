...
export function assertPhase7BIsolatedExerciseIdentity({ configuration, identity, input, exercise, trusted } = {}) {
  ...
  if (!expected.spacesPrefix.startsWith("phase7b-isolated/")) {
    throw guardError("PHASE7B_EXERCISE_SPACES_NOT_ISOLATED", "The Phase 7B exercise Spaces prefix is not isolated.");
  }
...
function normalizeTrusted(value) {
  ...
  const result = {};
  for (const field of ["environment", "ownerUserId", "datasetId", "databaseClusterId", "databaseName", "spacesBucket", "spacesPrefix", "providerWorkerId", "windowsHostId"]) {
