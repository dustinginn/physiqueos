...
function validate(name, value, config, input, now, maximumAge) {
  ...
  return ({
    backups: () => value.windowsEncryptedRestoreVerified === true && value.windowsIndependentReplicaVerified === true && value.postgresManagedBackupCurrent === true && value.postgresIsolatedRestoreVerified === true && value.spacesIndependentRestoreVerified === true && validReference(value.windowsBackupRef) && validReference(value.postgresBackupRef) && validReference(value.spacesBackupRef),
