...
describe("PostgreSQL Photo Event briefing read store", () => {
  it("uses three bounded queries and no compatibility runtime", async () => {
    ...
    const query = vi.fn(async (sql) => ({ rows: sql.includes("canonical_briefing_records") ? [{ payload: { briefing: { photoEventNarrative: { context: { activeGoal: { id: "goal-bound-to-event" } }, activeViews: [{ imageHref: `media://${mediaId}`, previousImageHref: "/api/private-evidence/founder/evidence/uploads/front.jpg" }] } } } }] : [] }));
    ...
    const goalCall = query.mock.calls.find(([text]) => text.includes("canonical_goal_records"));
    expect(goalCall[1]).toEqual(["owner", "goal-bound-to-event"]);
  });
  ...
  it("loads only the Photo Event's matched Confidence assessment", async () => {
    const query = vi.fn(async (sql) => ({ rows: sql.includes("canonical_briefing_records")
      ? [{ payload: { confidencePublication: { assessmentId: "assessment-one" },
          briefing: { photoEventNarrative: { context: { activeGoal: { id: "goal-bound-to-event" } }, activeViews: [] } } } }]
