...
describe("Workout canary audit", () => {
  it("proves candidate linkage, quarantine, and Logger authority for a canary window without private identifiers", async () => {
    const session = {
      ...
      payload: { id: "SECRET-SESSION-ID", evidence_type: "training", observed_at: DAY, captured_at: `${DAY}T12:00:00.000Z`, source: { application: "Training Logger", modality: "manual" },
        metadata: { activity_type: "Traditional Strength Training", logger_origin: "training_logger", logger_mode: "live", start_time: `${DAY}T10:01:00-07:00` },
        exercises: [{ name: "Squat", sets: [{ reps: 5, weight: 225 }] }] },
    };
    const canonicalEvidenceStorageMetadata = [{
      recordId: session.canonicalId,
      createdAt: `${DAY}T17:59:00.000Z`,
      updatedAt: `${DAY}T17:59:00.000Z`,
    }];
    const records = createInMemoryCanonicalRecordStore({
      ...
      storageMetadata: { canonicalEvidenceObjects: canonicalEvidenceStorageMetadata },
    });
    ...
    const summary = summarizeHealthKitWorkoutCanary({
      ...
      canonicalEvidenceObjects: snapshot.canonicalEvidenceObjects, canonicalEvidenceStorageMetadata,
      startLocalDate: "2026-09-24", endLocalDate: "2026-09-26", includeValues: false,
    });
  ...
  it("bundles a read-only audit and a workout-kind policy payload with baked identity", async () => {
    ...
    expect(entry).toContain("records.listStorageMetadata");
