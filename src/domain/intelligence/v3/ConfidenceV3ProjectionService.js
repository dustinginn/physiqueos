...
export function projectConfidenceV3({ goalContract, interpretation, authorityBindings = [], priorConfidence, evaluationContext }) {
  ...
  const dayChanged = Boolean(priorConfidence?.assessedAt &&
    assessedAt.slice(0, 10) !== priorConfidence.assessedAt.slice(0, 10));
  const temporalReassessment = dayChanged && goalContract.objectives.some((item) => item.forecast?.deadlineAt);
  ...
  let noMaterialChange = Boolean(priorConfidence?.goalConfidence && sameScope && !materialBindings.length && !temporalReassessment);
  ...
  if (sameScope && !temporalReassessment && materialBindings.length &&
  ...
  const projectionMode = noMaterialChange ? "continuity_hold" : authoritativeBreach ? "authoritative_guardrail_anchor" : authoritativeAchievement ? "achieved_state_anchor" : decisiveOutcome ? "decisive_state_anchor" :
    authoritativeOutcome ? "material_state_anchor" : !sameScope && priorConfidence?.goalConfidence ? "scope_revision_blend" :
      executionBindings.length ? "execution_update" : temporalReassessment ? "time_budget_update" : "limited_evidence_update";
  ...
  const movementBound = authoritativeOutcome || authoritativeBreach || authoritativeAchievement ? null : projectionMode === "scope_revision_blend" ? 15 :
    executionAuthority >= 0.5 ? 8 : executionAuthority >= 0.2 ? 4 : temporalReassessment ? 3 : 2;
