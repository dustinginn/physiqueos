...
describe("overall goal confidence parity",()=>{
  ...
  it("gives Home and the active-goal preview one canonical persisted truth without writing",async()=>{const before=fs.readFileSync(storePath,"utf8");const [home,preview]=await Promise.all([HomeBriefingService.getHomeBriefing(),getPhaseAwareActiveGoalPreview()]);const after=fs.readFileSync(storePath,"utf8");expect(after).toBe(before);expect(home.hero.confidence).toEqual(expect.any(Number));expect(preview.hero.confidence).toBe(`${home.hero.confidence}% confidence`);expect(preview.hero.confidenceBand).toBe(home.hero.confidenceState);expect(preview.hero.confidenceSource).toBe("canonical_confidence_v2_snapshot");expect(preview.hero.confidenceSource).toBe(home.hero.confidenceSource);expect(preview.hero.confidenceAssessmentId).toEqual(expect.any(String));});
  ...
  it("does not recalculate canonical confidence at a UTC date boundary",async()=>{
    ...
    expect(home.hero.confidenceSource).toBe("canonical_confidence_v2_snapshot");
