...
describe("phase-aware active goal preview",()=>{
  it("projects the canonical goal, phases, and baseline without raw lifecycle language",()=>{const result=composePhaseAwareActiveGoalPreview({user:{timeZone:"America/Los_Angeles"},goal,dexaScans:dexa,currentDate:new Date("2026-07-22T12:00:00Z")});expect(result).toMatchObject({goalId:"goal-build",phaseId:"p1",currentPhase:{id:"p1",goalId:"goal-build"}});expect(result.hero).toMatchObject({title:"Build Lean Mass",status:"Active Goal",destination:"Build 10 lb of lean mass by October 31, 2026"});expect(result.journey.map(x=>[x.name,x.status,x.color])).toEqual([["Establish Maintenance","Active","orange"],["Lean Mass Build","Planned","green"]]);expect(result.journey[0].progress).toBe("Week 1 of 4");expect(result.journey[1]).toMatchObject({progress:"0 of 10 lb measured",support:"Awaiting next DEXA"});expect(result.evidence.goalBaseline).toMatchObject({bodyFat:"7.7%",leanMass:"147.5 lb"});expect(result.evidence.phaseStart).toBeNull();expect(JSON.stringify(result.readiness)).not.toMatch(/transitionPolicy|timingMode|lifecycle/);});
