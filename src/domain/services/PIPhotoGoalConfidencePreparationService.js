...
export function createPIPhotoGoalConfidencePreparationService({
  readService,
  now = () => new Date(),
} = {}) {
  ...
  return Object.freeze({
    async prepare(request = {}) {
      if (request.triggerType !== PIGoalConfidenceTriggerType.PHOTO_EVENT) {
        return { status: "not_eligible", reason: "unsupported_photo_trigger" };
      }
      let publicationCommand = null;
      const refresh = createPIGoalConfidenceRefreshService({
        readService,
        now,
        persistenceService: { publish: async (command) => {
          publicationCommand = structuredClone(command);
          return { status: "published", committed: false,
      ...
      const result = await refresh.refresh({
        ...
        publicationReason: request.publicationReason ??
          `Photo confidence publication ${request.occurrenceId}`,
        preparedPIReasoning: {
      ...
      if (result.status === "matched") return {
        status: "visual_evidence_already_consumed",
      ...
      return {
        ...
        visualContributor: result.contributors.find(
          (item) => item.domain === "photos") ?? null,
        receipt: result.receipt,
        publicationCommand,
      };
    },
