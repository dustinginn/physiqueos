...
const EVENT_VERSION = "photo_event_v3_3_0";
...
export function composePhotoEventNarrative({ session, goal = null, latestDexa = null, priorDexa = null, baselineDexa = null, milestone = null, executionSupport = {}, confirmationIntent = null, completionComparisons = null, visualCriterionComplete = "uncertain", generatedAt = new Date().toISOString() } = {}) {
  ...
  return {
    ...
    goalMeaning: completionCopy?.goalMeaning ?? `The photos support continued progress toward your ${goalTitle.toLowerCase()}, while the next DEXA remains the better measure of body-composition change.`,
    coachingDirection: completionCopy?.coachingDirection ?? `Stay the course into ${milestoneText}. Keep the next photo set as consistent as possible so subtle abdominal changes are easier to judge.`,
    ...
    cardContent: {
      hero: { id:`${narrativeId}_hero`, title:completionCopy?.title ?? "Today’s photos suggest continued progress toward visible abs.", body:completionCopy?.summary ?? "The clearest change is a tighter waist in the front view, while both rear views suggest upper-body muscle has been maintained." },
...
export function createPhotoEventNarrativeService({ repositories, now = () => new Date() }) {
  return {
    ...
    async getOrCreateResult({ userId, sessionId, preview = false }) {
      ...
      if (existing?.briefing?.version === EVENT_VERSION && !preview) return {
      ...
      const narrative=composePhotoEventNarrative({session,goal,latestDexa,priorDexa,baselineDexa,executionSupport,confirmationIntent:session.confirmationIntent,completionComparisons,milestone:{label:"DEXA on Saturday, Jul 18 at 7:30 AM",date:"2026-07-18"},generatedAt:now().toISOString()});
      if (!narrative) return {
        status: "blocked",
