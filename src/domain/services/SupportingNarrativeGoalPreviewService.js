...
function composeMaintenance({config={},data={}}={},goal){
  ...
  const transitionReady=data.lifecycleState==="transition_ready";
  const nearing=transitionReady||/entering|target range|on track/i.test(data.status??"");
  if(transitionReady)return base({goal,config,data,hero:{state:"Ready for Next Phase",conclusion:`The latest DEXA is ${latestScan?.bodyFat?.toFixed(1)}%, below the 8–9% maintenance range. The cut threshold has been reached; the next decision is how to establish maintenance.`,estimate:null},map:{summary:"The measured cut result is below the intended maintenance range.",stops:[stop("complete","Cut Threshold","Reached"),stop("current","Today",`${latestScan?.bodyFat?.toFixed(1)}% Body Fat`),stop("next","Next Decision","Establish Maintenance"),stop("destination","Goal","Stable 8–9% Body Fat")]},ground:[latestScan&&chapter("DEXA Confirmed",`${latestScan.bodyFat.toFixed(1)}% is below the intended 8–9% maintenance range.`)],road:[road("transition","Establish Maintenance","Choose a deliberate transition without automatically changing the current plan.")],criteria:[criterion("8–9% Body Fat Range","Below Range"),criterion("Stable Weight in Target Range","Remaining")],strategy:{conclusion:"Use the current result to make the maintenance decision deliberately.",constraint:"Do not imply that maintenance is already established."},turning:[latestScan&&turn(latestScan.date,"Cut Threshold Reached",`${latestScan.bodyFat.toFixed(1)}% made the next-phase decision current.`)],confidence:{summary:"DEXA provides a valid current measurement.",watching:"Stability after the deficit ends."},transition:{title:"What comes next",state:"Ready for Next Phase",body:"The next discussion is how to establish maintenance; this patch does not activate or create that phase."}});
...
function composeLeanMass({config={},data={}}={},goal){
  ...
  const achieved=data.lifecycleState==="achieved";
  const stable=achieved||/stable|on track|protected/i.test(data.status??"");
  if(achieved){const evidence=data.achievementEvidence??{};return base({goal,config,data,hero:{state:"Achieved",conclusion:`Lean-tissue preservation is confirmed at ${latestScan?.leanMass?.toFixed(1)} lb, ${signed(evidence.delta)} lb from the May 24 baseline.`,estimate:null},map:{summary:"The Jul 18 DEXA completed the preservation check.",stops:[stop("complete","May 24 Baseline",`${evidence.baselineLeanMass?.toFixed(1)} lb`),stop("complete","Jul 18 Result",`${evidence.currentLeanMass?.toFixed(1)} lb`),stop("current","Today","Achieved"),stop("destination","Goal","Lean Mass Preserved")]},ground:[chapter("Preservation Confirmed",`${evidence.currentLeanMass?.toFixed(1)} lb of lean tissue was retained through the cut.`)],road:[road("decision","Next Goal Decision","Review the next objective without creating it yet.")],criteria:[criterion("Lean Mass Preserved Through Cut","Achieved")],strategy:{conclusion:"The preservation objective is complete.",constraint:"Do not automatically activate a new goal."},turning:[turn(evidence.baselineDate,"Lean Mass Baseline",`${evidence.baselineLeanMass?.toFixed(1)} lb established the reference.`),turn(evidence.currentDate,"Preservation Achieved",`${evidence.currentLeanMass?.toFixed(1)} lb confirmed the outcome.`)],confidence:{summary:"Comparable DEXA measurements establish the result.",watching:"The next-goal decision."},transition:{title:"What comes next",state:"Decision Ready",body:"Lean-mass preservation is achieved. The next objective remains a separate decision."}});}
...
function signed(value){const numberValue=Number(value);return Number.isFinite(numberValue)?`${numberValue>0?"+":numberValue<0?"−":""}${Math.abs(numberValue).toFixed(1)}`:"0.0";}
