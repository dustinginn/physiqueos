...
describe("HealthKitObservationService", () => {
  it("derives stable source identity independently from delivery batches", () => {
  ...
  it("keeps cardio as a canonical candidate until Evidence eligibility is independently available", () => {
  ...
  it("rejects an additive or workout-only Activity summary", () => {
    const invalid = activitySummary();
    invalid.activitySummary.aggregationScope = "workout_only";
    expect(() => normalize("batch-one", [invalid])).toThrowError(expect.objectContaining({
...
function workout({ activityType = "Traditional Strength Training" } = {}) {
  return {
...
function activitySummary() {
  return {
    ...
    activitySummary: {
      ...
      sourceRevision: 1,
      dailyActivity: { move_calories: 700, exercise_minutes: 45, stand_hours: 12 },
    },
  };
}
