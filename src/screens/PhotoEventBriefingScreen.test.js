...
describe("PhotoEventBriefingScreen V2",()=>{
  ...
  it("opens one reusable in-place touch viewer without Gallery navigation",()=>{expect(source).toContain("setViewer({items,index})");expect(source).toContain('role="dialog"');expect(source).toContain("onTouchStart");expect(source).toContain("onTouchEnd");expect(source).toContain("[touch-action:pinch-zoom]");expect(source).not.toContain("Open in Gallery");});
  it("restores scroll position and cycles the supplied gallery items",()=>{expect(source).toContain("scrollPosition.current=window.scrollY");expect(source).toContain("window.scrollTo");expect(source).toContain("items.length");});
  ...
  it("gates paging during zoom and preloads adjacent photos",()=>{expect(source).toContain("setZooming(true)");expect(source).toContain("if(zooming)");expect(source).toContain("const preload=new Image()");expect(source).toContain("preload.src=items[index].imageHref");});
