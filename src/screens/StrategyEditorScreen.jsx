...
export default function StrategyEditorScreen({ action, model }) {
  ...
  return <main className="app-surface min-h-screen"><form action={formAction} className="mx-auto max-w-[393px] space-y-6 px-4 pb-28 pt-10">
    ...
    <header className="space-y-2"><p className="text-xs font-extrabold uppercase tracking-[.12em] text-[var(--primary)]">Operating Plan</p><h1 className="text-3xl font-extrabold text-[var(--text-primary)]">{model.title}</h1><p className="text-sm leading-6 text-[var(--text-secondary)]">{model.strategyType==="briefings"?"Choose when coaching updates are available and whether you want to be notified.":"Update the strategy itself. Days, times, and reminders remain in Execution."}</p></header>
...
function NutritionFields({ model }) {
  return <div className="space-y-4">
    ...
    <Section title="Nutrition approach"><Select defaultValue={model.calorieStrategy} label="Intake approach" name="calorieStrategy" options={model.options.calorieStrategy}/><Select defaultValue={model.carbohydrateStrategy} label="Carbohydrate approach" name="carbohydrateStrategy" options={model.options.carbohydrateStrategy}/><Select defaultValue={model.fatStrategy} label="Fat approach" name="fatStrategy" options={model.options.fatStrategy}/></Section>
  </div>;
}
...
function TrainingFields({ model }) {
  return <div className="space-y-4">
    ...
    <Section title="Strategy approach"><Select defaultValue={model.progression} label="Progression" name="progression" options={model.options.progression}/><Select defaultValue={model.phase} label="Current phase" name="phase" options={model.options.phases}/></Section>
  </div>;
}
