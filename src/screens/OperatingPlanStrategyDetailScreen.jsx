...
export default function OperatingPlanStrategyDetailScreen({ detail }) {
  return <main className="app-surface min-h-screen"><div className="mx-auto max-w-[393px] px-4 pb-28 pt-10">
    ...
    {!detail ? <Unavailable/> : <>
      <header className="mb-6 space-y-2"><p className="text-sm font-semibold uppercase tracking-[0.12em] text-[var(--primary)]">{detail.eyebrow}</p><h1 className="text-3xl font-extrabold leading-tight text-[var(--text-primary)]">{detail.title}</h1><p className="pt-1 text-[10px] font-extrabold uppercase tracking-[.1em] text-[var(--text-muted)]">Purpose</p><p className="text-sm font-medium leading-6 text-[var(--text-secondary)]">{detail.purpose}</p></header>
      <div className="space-y-4">
        ...
        <Card className="space-y-3"><div className="flex items-center gap-3"><IconBadge className="rounded-full" color="primary" icon={Target} size="sm"/><h2 className="font-extrabold text-[var(--text-primary)]">Supporting</h2></div><p className="text-sm font-semibold text-[var(--text-secondary)]">{detail.goal ?? "Your current direction"}</p></Card>
