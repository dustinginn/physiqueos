...
The isolated database was independently read from the existing worker and is exact at `000004`, with no authority/transfer row and only synthetic data (42 collections, 370 records, 764 relationships, three versioned objects / 111 bytes). Migration/initialization and provider web/worker boot remain open gates until their bounded provider-side acceptance completes; no production mutation is authorized.
