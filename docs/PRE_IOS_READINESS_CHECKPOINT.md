...
Migration `000005` now represents compatibility as a distinct non-authoritative state rather than a production state with a flag. Compatibility access requires the exact isolated database and environment, retains Windows public/canonical authority, forbids a production operation and first-write boundary, and is revalidated before web commands, media completion, and worker polling. Live App-spec inspection is closed: deployment `0d27de79-169a-4fda-a16c-ad868d46b7e4`, its one-web/one-worker topology, web-only route, six web and seven worker encrypted-variable names, eight alerts, and spec digest `2ADB019D77AB20F6AD58AFA5DB7174D539EF2D41A8ED0A04CFB6A5E22BD62602` were independently captured without secret values. The full-product template preserves all eight alerts and the same $30.15/month topology.

The isolated provider database is now at `000005` with authority `provider-compatibility-nonauthoritative`, no migration operation, no transfer receipt, no first provider canonical write, and only the synthetic owner. Current-shape package validation covers 39 required collections, 358 records, 739 relationships, application context, and three private/versioned media objects totaling 111 bytes. The retained 12 records from three superseded synthetic context collections are noncanonical compatibility history and are not read by the current 39-collection runtime. The compatibility worker has completed a bounded healthy boot with production migration execution disabled. Full web acceptance additionally requires the Home route to resolve through the application composition/read-model boundary; a direct legacy runtime-store read is a blocking failure, not an allowed fallback.

The pre-remediation provider-side read established the `000004`/no-authority baseline before the bounded `000005` preparation described above. Exact-checkpoint provider web and worker boot remain release gates until their bounded acceptance completes; no production mutation is authorized.

The stronger artifact gate rejects Founder owner identifiers without caller-supplied signatures. A deployment-equivalent web artifact and the deterministic worker module/dependency closure both require a zero-violation scan; non-runtime scripts/tests, dependency documentation/examples, and legacy seed default exports are no longer eligible to enter those runtime artifacts.

### Provider-build lifecycle safety repair

A provider full-runtime build was previously launched from the canonical Windows checkout and wrote the default `.next` destination. Windows was recovered to its accepted artifact, and the overwritten provider output was retained as noncanonical incident evidence. The repaired lifecycle makes that invocation structurally fail before compilation: provider mode in `next.config.mjs` requires an explicitly identified isolated checkout, exact source/build identities, and a destination confined to that checkout. Canonical `.next`, the retained rollback/fallback/failed-overwrite evidence, and dynamically discovered recovery/staging directories are deny-listed by resolved filesystem identity, including aliases and reparse paths.

The committed provider preflight wrapper performs read-only before/after verification of the Windows build/source, bounded artifact identity, task PID/start time, canonical working directory, and every recovery artifact. A mismatch blocks acceptance without automatic repair. Its executable surface can build and inspect isolated provider artifacts only; it cannot call the Windows stop/start/deploy, Scheduler, promotion, fallback, or cleanup paths. App Platform deployment and rollback remain a separate provider-only lifecycle and cannot mutate Windows state.
