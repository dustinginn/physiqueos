...
Canonicalization is off unless the server-owned `healthKitConfiguration` record `healthkit_canonical_daily_activation_policy` is `status: "enabled"` with schema `healthkit-canonical-activation-policy-v1`, explicit `domains` (`activity`, `nutrition`), and an exact inclusive `effectiveLocalDate`..`endLocalDate` window of at most 7 local dates -- **or** `openEnded: true` with `effectiveLocalDate` and no `endLocalDate` at all, for genuinely permanent, forward-only daily-driver operation (still no historical backfill: a date before `effectiveLocalDate` is refused exactly as the bounded case). A record naming both `openEnded: true` and an `endLocalDate` is invalid. `historicalBackfill` is always false and `strategicEvidenceEligibility` is always `quarantined`; neither is a parameter, and a record that says otherwise is invalid. An invalid or unrecognized policy never throws: it resolves to disabled so a delivery is never turned into a permanent Native 400. The Workout activation policy (a separate record and kind) does not support `openEnded`; it stays proving-period-bounded only, by design.

The only writer is `HealthKitActivationPolicyRunner` (bundled by `scripts/operations/buildHealthKitPayload.mjs --kind policy [--open-ended]`, run through the accepted console runner): dry-run first, `expected` facts baked into apply, a drift fence, exactly two new/updated rows (the policy and one audit row), owner advisory lock, in-transaction verification, rollback on any failed invariant. `deactivate` sets the policy to disabled and never deletes a canonical day, source observation, or Evidence row. Windows are never widened in place; deactivate first.
...
**Operational consequence:** if an observation arrives while the window does not yet cover its date (for example, before an open-ended policy correcting a narrower prior window has been applied), that specific raw observation is permanently barred and will never canonicalize, even after the policy is fixed -- a replay of the exact same source identity just copies the stored decision forward. It is harmless, permanent history, not a stuck state to work around. The safe recovery is: correct the policy first, then have Native perform one fresh sync. A fresh sync is never a replay of the barred observation (each device-scoped `sourceRevision` is new), so it gets a brand-new identity and a fresh assessment against the corrected policy, and the day canonicalizes cleanly with no duplication (only one canonical day can ever exist per domain and date).
...
HealthKit workouts remain source observations. Production Workout canonicalization is OFF until its own server-owned policy record, `healthkit_workout_canonical_activation_policy` (schema `healthkit-workout-activation-policy-v1`, domain `workout` only, quarantined, no backfill, `linkAutoConfirm` false), is enabled. The window is either a bounded canary (exact inclusive `endLocalDate`, at most 3 local dates) or prospective (`openEnded: true`, no end date, forward-only from `effectiveLocalDate`). `families` scopes it to a non-empty subset of `cardio`/`strength` (absent = both); a workout of a family outside the scope is stored raw as `workout_canonicalization_deferred / family_not_in_activation_scope` and, like every raw workout stored under a policy, is never reconsidered by a later policy. The record is independent of the Activity + Nutrition policy: separate id, separate window, separate audit rows, and the activation operation proves by digest that it never reads or writes the other. Like the daily policy it fails closed and never throws, and a raw workout stored before activation is never reconsidered (activate first, then sync).
