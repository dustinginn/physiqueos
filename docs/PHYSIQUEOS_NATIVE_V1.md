...
Build 43 physical acceptance passed every Progress Photos step up to upload initiation and then failed at the real upload with `413 MULTIPART_REQUEST_TOO_LARGE`. The cause was application-owned: Native decoded every HEIC original and re-encoded it as a maximum-quality JPEG (inflating it well past the original), then sent the whole multi-photo set in one multipart request that the Server bounds at 51 MiB. Nothing durable was created. Build 44 replaces the aggregate upload with staged media; version/build stay `1.0 (43)` until release.
...
**Originals.** `EvidenceAttachmentLoader.stagedPhotoRepresentation` preserves JPEG, PNG, WebP, HEIC, and HEIF originals byte for byte. The Build 43 `serverCompatiblePhoto` HEIC-to-JPEG re-encode is deleted. A photo above the 32 MiB per-photo ceiling is refused with a clear message, never recompressed. Because the Server runtime cannot decode HEVC, each HEIC/HEIF original is accompanied by one bounded JPEG rendition (`PhotoAnalysisDerivativeGenerator`: ImageIO thumbnail path, 2048 px longest edge, quality 0.9, orientation baked in, never a full-resolution decode) that the Server uses only for browser display and the vision interpreter; the canonical photo remains the original.
