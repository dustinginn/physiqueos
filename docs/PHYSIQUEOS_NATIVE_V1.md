...
## 67. Native Operating Plan read/browse/sandbox-editor vertical (2026-08-31)
...
`ProtocolDetailScreen.jsx`, `StrategyDomainScreen.jsx`,
`PeptideExecutionManagementService.js`/`PeptideDosingStrategyModel.js`,
`SupplementStrategyEditorScreen.jsx`, `TrackingSupportService.js`, their
tests, and `src/data/seed/*` for synthetic sample values. Several audited
routes turned out to be dead code kept for historical/test reference —
`energy/new` unconditionally redirects, `protocols/page.js` unconditionally
redirects, and top-level `operating-plan/actions.js`'s `saveOperatingPlan`
is imported by nothing — and were **not** ported as if they were live.

### Reachable web inventory and Native disposition

| Operating Plan surface | Current product content | Native disposition |
|---|---|---|
...
`Contracts/OperatingPlanReadModel.swift` holds every Operating Plan
read/editor model, field-for-field against the audited web shapes, plus the
one explicitly-documented native-only phase-history extension above and
pure validation functions mirroring `StrategyEditorService.js`'s exact
ranges. `Networking/OperatingPlanSandboxStore.swift` is the fixture-backed,
...
introduced, matching the established sandbox-editor convention rather than
the async-`API`-plus-`ViewModel` convention used by read-only verticals like
Goals (Operating Plan's edits are genuinely local and synchronous, so that
heavier shape would have been unearned complexity).

Visual composition mirrors the web's actual Operating Plan screens, not
Goals' gradient "atmospheric" card language: `Card.jsx` + `IconBadge.jsx`
are plain, densely stacked, compact rows, so `Presentation/OperatingPlan/
OperatingPlanComponents.swift` composes the existing shared `CardContainer`/
`IconBadge`/`SectionHeading` rather than introducing new visual chrome.
Section/tone-to-icon mapping is copied from `OperatingPlanScreen.jsx`'s own
icon map and `IconBadge.jsx`'s tone slots, not invented.

### Navigation

`YouPlaceholderView` (the "You" tab, `route: "profile"`) gained one real
doorway — Operating Plan — matching `YouScreen.jsx`'s own entry point into
`/profile/operating-plan`; this is the smallest source-faithful route into
the vertical the current navigation architecture needed. The rest of
`YouPlaceholderView` (Operating Status, Goals doorway, Integrations
doorway) remains an honest placeholder — building out the full Profile/You
screen was not this slice's scope. `RootTabView` gained a `youPath`
`NavigationStack` (previously bare), matching the pattern every other tab
...
`evidenceIntake`/`localEvidenceReview`). Peptide execution and Recovery
support editing use an in-place `isEditing` toggle on one view rather than
a second push destination, deliberately mirroring the web's own `?edit=1`
query-param pattern on the same route.
...
## 2026-09-19 — Build 44 candidate: staged Progress Photos transport (branch `codex/build44-progress-photos-staged-native`, not yet released)

Build 43 physical acceptance passed every Progress Photos step up to upload initiation and then failed at the real upload with `413 MULTIPART_REQUEST_TOO_LARGE`. The cause was application-owned: Native never transferred a HEIC original as-is — PhotosPicker's default `.automatic` encoding policy offers a HEIC asset as its compatible JPEG transcode first, and anything not already JPEG/PNG/WebP was additionally decoded and re-encoded as a maximum-quality JPEG (inflating it well past the original) — and the whole multi-photo set went in one multipart request that the Server bounds at 51 MiB. Nothing durable was created. Build 44 replaces the aggregate upload with staged media; version/build stay `1.0 (43)` until release.

**Transport.** `StagedPhotoIntakeCoordinator` stages the set on disk (`FileStagedPhotoIntakeStore`, Application Support, complete file protection, atomic writes) as one durable `StagedPhotoIntakePlan`: the deterministic submission identity (same `ProductionIdempotencyKeyStore` scope/signature discipline as every other write), the Founder's session conditions and confirmed pose identities (the exact `photoIdentitiesJson` text the multipart transport sends), and the expected artifact set with Server-derived identities `artifact_{submissionIdentityWithoutDashes}_{ordinal}`. It declares the intake with `POST /api/v1/native/evidence/intakes/staged` (JSON, `Idempotency-Key` = submission identity) and transfers each artifact alone with `PUT /api/v1/native/evidence/intakes/{intakeId}/artifacts/{artifactId}` as a raw image body under its declared MIME type. The Server verifies length, SHA-256, and container bytes against the declaration, reports per-artifact `expected`/`stored` progress and `mediaComplete` on every response, and completes media only when every declared artifact is verified. Server progress always wins over local memory: a lost acknowledgement is resolved by the next declaration replay or status read; a re-sent artifact is acknowledged `already_stored` without a second object. Transient failures leave the plan pending; `resume` re-declares and transfers only what is missing, across navigation, suspension, termination, and reboot. A validation rejection marks the artifact rejected and is never retried silently (dismiss/discard and choose again). Local bytes are discarded only after the Server reports complete media. Media completion is not acceptance of evidence: the follow-up is the same Server-driven Evidence Review transition (`readyReview`/`awaitReadyIntake`) every intake uses, and nothing in this transport completes a priority, creates a review, or writes canonical state.

**Originals.** `EvidenceAttachmentLoader.stagedPhotoRepresentation` preserves JPEG, PNG, WebP, HEIC, and HEIF originals byte for byte. The Build 43 `serverCompatiblePhoto` HEIC-to-JPEG re-encode is deleted. The picker now requests `preferredItemEncoding: .current`, so a HEIC asset is delivered as its own bytes rather than the system's compatible JPEG transcode: the simulator's Photos log shows the default policy registering `public.jpeg (compatible)` ahead of `public.heic (current)` for HEIC assets, and the first staged walkthrough transferred transcoded JPEGs as a result. The declared container is read from the bytes (`detectImageContainer`, the same signatures the Server verifies) and never from the picker's type label, so a declaration cannot disagree with what is transferred. A photo above the 32 MiB per-photo ceiling is refused with a clear message, never recompressed. Because the Server runtime cannot decode HEVC, each HEIC/HEIF original is accompanied by one bounded JPEG rendition (`PhotoAnalysisDerivativeGenerator`: ImageIO thumbnail path, 2048 px longest edge, quality 0.9, orientation baked in, never a full-resolution decode) that the Server uses only for browser display and the vision interpreter; the canonical photo remains the original.

**UI.** The Build 43 screen is unchanged: independent Progress Photos entry, multi-photo selection, real per-photo previews, Orientation/Contraction/Pose, REVIEW/CONFIRMED, session date, the 2x2 Session Conditions grid, original/unedited confirmation, Upload. Goal role stays hidden and defaults to `supporting`. Added: per-photo transfer copy during upload, a "Photo upload waiting" card with Resume/Discard when a staged set is pending, and a Resume affordance on the failure state. Post-upload pose editing, the Morning tri-state, Lighting, Location, and Session notes remain out.

**Verification.** `StagedPhotoIntakeTests` covers representation preservation, the bounded derivative, deterministic plan identity and durability, ordered transfer and completion, transient failure with relaunch resume, lost acknowledgement, already-stored replay, failure before Server receipt, rejection without silent retry, incomplete media never reporting complete, dismissed-predecessor identity rotation, status decoding, and the Server-driven review transition.

## 2026-09-20 — Build 45 candidate: Apple ProRAW DNG originals through the container registry (branch `codex/build45-progress-photos-proraw-native`, not yet released)

Build 44 physical acceptance failed locally with "One of the selected photos is not a JPEG, PNG, WebP, or HEIC image." before any plan was persisted or any request left the device. AirDropped originals proved the set: five **Apple ProRAW DNG** files (iPhone 17 Pro, big-endian TIFF with `APPLEDNG`, DNGVersion 1.6, 4828×6438, 40.0–46.2 MB) captured with Camera set to High Efficiency/24 MP — ProRAW is a per-shot Camera toggle that overrides that setting. Build 43's `413` had the same cause: the DNGs were decoded by `UIImage` and re-encoded as q1.0 JPEG into one aggregate body.

**Container registry, not a format branch.** `EvidenceAttachmentLoader.photoContainers` mirrors the Server's `ImageContainerDetection` registry: each accepted container carries its MIME type, extension, whether display/analysis can consume it directly, and its transport size class. `stagedPhotoRepresentation` asks the registry; `requiresAnalysisDerivative` is `!directlyConsumable`; `StagedPhotoIntakePlan.originalMaximumBytes(for:)` is 32 MiB for compressed originals and 48 MiB for raw ones (the Server's application-owned ceiling on the artifact PUT path, 8% over the largest real ProRAW). DNG is simply the first raw entry. Originals stay byte-for-byte; the existing ImageIO thumbnail path produces the 2048 px q0.9 JPEG derivative for any non-consumable original (verified locally on the five Founder files: 1536×2048, ~0.6 MB, deterministic).

**Detector parity.** `detectImageContainer` is now a faithful port of the Server's: ISO BMFF `ftyp` box size validated (16–4096 bytes, within the data), major **and compatible** brands scanned against the Server's HEIC/HEIF brand sets (so a generic `mif1`/`MiHE` major brand with `heic` compatible is HEIC), plus TIFF `II*\0`/`MM\0*` with a bounded first-IFD walk (≤ 1024 entries, every offset checked) that accepts only a four-BYTE DNGVersion with major version 1. Plain TIFF, malformed headers, and unknown brands are refused.

**Diagnostics.** An unsupported photo now fails with `UnsupportedPhotoDiagnostic` — selected ordinal, the picker's reported type, byte length, and a signature-family classification (`tiff:no-dng-version`, `iso-bmff:<brand>`, `unknown:<4 hex bytes>`) — logged through `os.Logger` (`com.physiqueos.native` / `ProgressPhotos`) and surfaced as "Photo N is a TIFF image without Apple ProRAW information, which PhysiqueOS can't accept. Progress Photos can be JPEG, PNG, WebP, HEIC/HEIF, or Apple ProRAW (DNG)." Size refusals name the photo and its container's ceiling. No file names, paths, or image bytes are logged or shown.

**Unchanged.** The Build 43/44 Progress Photos screen, the staged plan/retry/idempotency model, HealthKit, and version/build `1.0 (44)` until release. The Server must deploy its matching registry change before Build 45 ships.

## 2026-09-20 — Progress Photos pose contract, readable review, and honest confirmation (branch `codex/photos-semantic-contract-native`, Build 45 metadata unchanged, not yet released)

Real Build 45 ProRAW acceptance reached an Evidence Review that said "5 photos · 1 pose still to choose". Photo 3 had been confirmed as Rear + Relaxed + Double Biceps, a combination the Server does not recognize (Double Biceps is a Rear, Flexed pose). The Founder confirmed the intended identity is Rear + Flexed + Double Biceps and that the combination came from a control defect.

**One canonical pose contract.** Orientation, contraction, and pose variant are not independent. Native no longer keeps its own rules: `ios/PhysiqueOS/Contracts/ProgressPhotoPoseContractData.swift` is generated (`ios/Scripts/generate_pose_contract.py`) from `contracts/progress-photo-pose-contract.v1.json`, which the Server generates from its pose vocabulary and publishes in its contract manifest. `ProgressPhotoPoseContract` derives every choice offered, every adjustment, and the "Confirm pose" gate from that table. A test compares the generated table to the shared JSON, so drift fails the build.

**Interaction.** The Contraction and Pose menus offer only what exists for the chosen orientation; Lat Spread, Side Chest, and Other are no longer offered. Choosing Double Biceps sets Contraction to Flexed and shows "Double Biceps uses Flexed." Choosing Relaxed while Double Biceps is set resets Pose to Standard and says why ("Double Biceps uses Flexed, so Pose was reset to Standard."); the Founder's choice is honored and the dependent choice moves, matching the menu-driven pattern of the rest of the screen without a modal. A change the contract cannot satisfy at all (Double Biceps on a Front photo, Flexed on a Side photo) is refused with a one-line explanation. Every adjustment clears the photo's confirmation, "Confirm pose" is enabled only for a canonical combination, submission requires it, and the serializer refuses to send anything else, using the contract's own spelling (a generic Side is `side_unspecified`). A pose Native lets the Founder confirm can therefore never later become "pose still to choose".

**Review.** The photo session's Goal relationship shows the resolved Goal's name, or plain language (`Needs session review`, `No goal linked`), never the raw `needs_review` enum.

**Confirmation.** The Server now refuses a photo review that cannot proceed with a 400 (`PHOTO_POSE_UNRESOLVED`, `PHOTO_SESSION_DETAILS_UNRESOLVED`) before recording any confirmation receipt. Native shows that as a failure ("This photo review can't be confirmed yet…"), never "Confirmation accepted", and a retry after correction reuses the same idempotency identity because the refusal recorded nothing.

Not changed: the staged-media transport, originals-as-canonical, HealthKit, and the version, which stays 1.0 (45) until a release is separately authorized. There is no post-upload pose editor: a mistake caught after upload is handled by dismiss and re-upload, or by the Server's bounded repair for the one historical review.
