...
Watermark persistence is now wired. The executor builds `buildEvidenceSettlementWatermarkV1` from the gate's decision and passes it to the generator as an additive `settlement` input; the generator attaches it to the artifact as one optional JSON member, `artifact.evidenceSettlement` (no schema migration). It carries the evidence window and cutoff, the canonical timezone and its authority, per-domain readiness (coverage, canonical record id, revision), settled/unsettled domains, `deadlineFallback`, `coverageReadFailed`, the earliest-publish and hard-deadline instants, a `closeoutReceipt` (always `null` today: no receipt endpoint exists), the generation timestamp (the artifact model does not distinguish generation from publication, so there is no separate publish timestamp) and a sha256 integrity envelope. `DailyBriefingRepository` deep-freezes it on write and on readback, and keeps the OLDEST watermark of an occurrence across replacement, retry and duplicate writes. Artifacts that predate the field simply do not have it and read exactly as before. A window with no HealthKit-backed readiness domain persists an honest `settlementApplicable: false` watermark.

A settlement coverage read failure now fails closed: wait/retry (`coverage_read_failed`) until the hard deadline, then generate best-available with `deadlineFallback: true` and `coverageReadFailed: true` (every domain `unknown_coverage_read_failed`). See `BriefingSettlementObservability.js` for the emitted `briefing_settlement.*` lifecycle and each event's dedup rule. `closeout_requested` is declared but not emitted (no request path exists); the Server emits `closeout_eligible`. `recordDeviceCloseoutReceiptV1` still has no Native->Server endpoint; that remains a separate, explicitly authorized follow-up.
