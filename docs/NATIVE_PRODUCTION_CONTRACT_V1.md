...
| Evidence Review dismiss | `evidence-review.dispose.v1` | approved Nutrition, Activity, Training, DEXA, or Progress Photos review ID; `If-Match` is required; marks pending evidence discarded without creating or modifying canonical history |

DEXA PDF and Nutrition, Activity, Training, or Progress Photos intake use `POST /api/v1/native/evidence/intakes` as `multipart/form-data`; status is read at `GET /api/v1/native/evidence/intakes/{intakeId}`. `Idempotency-Key` must equal the UUID submission identity. DEXA accepts exactly one signature-validated PDF. Nutrition, Activity, and Training accept one to four signature-validated PNG, JPEG, or WebP screenshots. Progress Photos accepts one or more signature-validated images plus an explicit Founder-confirmed pose identity for every file, session time of day, and the original/unedited confirmation. The foreground stores verified artifacts and returns a durable processing receipt; provider worker interpretation stages the ordinary Evidence Review. Native edits DEXA measurements there and confirms DEXA and Progress Photos through the same canonical lifecycle as web. Training Logger can bind one exact, versioned Training review to the exact committed session; the resulting private screenshots remain discoverable through authenticated opaque media descriptors on that session.

Progress Photos may instead use the staged-media transport, which exists because a full-resolution multi-photo set does not fit one bounded multipart request. `POST /api/v1/native/evidence/intakes/staged` (`application/json`, at most 64 KiB, `Idempotency-Key` equal to the UUID submission identity) declares the intake once: the session conditions and Founder-confirmed pose identities exactly as the multipart transport takes them, plus the expected artifact set. Each artifact is declared with its Server-derived identity `artifact_{submissionIdentityWithoutDashesLowercased}_{ordinal}` — the submission UUID with dashes removed and its hex lowercased whatever case the client's UUID uses, so the declaration and the artifact PUT path share exactly one canonical identity (ordinals contiguous across the whole set), role (`original` or `analysis_derivative`), file name, MIME type, byte length, and SHA-256. Originals accept JPEG, PNG, WebP, HEIC, and HEIF at most 32 MiB each, and Apple ProRAW DNG (`image/x-adobe-dng`, recognized from its TIFF header and DNGVersion tag, never from the label) at most 48 MiB each; every original is preserved verbatim. An original whose container display and analysis cannot read directly (HEIC, HEIF, DNG) must be accompanied by exactly one `image/jpeg` analysis derivative (at most 8 MiB, naming its original through `derivativeOf`) because this runtime decodes neither HEVC nor RAW; the container registry (`ImageContainerDetection`) decides which containers those are, so no transport or lifecycle rule names a format. At most 24 originals. Each artifact is then transferred alone with `PUT /api/v1/native/evidence/intakes/{intakeId}/artifacts/{artifactId}` as a raw image body under its declared MIME type with a required `Content-Length`; the Server verifies owner, intake, declared identity, length, SHA-256, and the image container bytes before cataloguing the object. Replaying the declaration returns the same intake with current progress; a declaration under the same identity with different content is `409 EVIDENCE_INTAKE_IDENTITY_CONFLICT`; re-sending an already stored artifact is acknowledged as `already_stored` without a second object; two concurrent transfers of one artifact converge on one object. `GET /api/v1/native/evidence/intakes/{intakeId}` reports `mediaState`, `mediaComplete`, and per-artifact `expected`/`stored` progress so a relaunched client resumes only what is missing. Media becomes `stored` only when every declared artifact is verified; until then `status` stays `processing` and interpretation never runs. Completion enqueues the same interpretation work as the multipart transport, so the Evidence Review, `evidence-review.commit.v1`, canonical PhotoSession, priority reconciliation, and Photo Event/Briefing lifecycle are unchanged. The canonical photo is always the original; the derivative is referenced as `analysis_storage_path` for browser display and the vision interpreter only. The Web client currently remains on the multipart transport; the staged contract is not Native-specific. Pose identity is governed by one canonical compatibility contract (`contracts/progress-photo-pose-contract.v1.json`, generated from `progressPhotoPoseVocabulary.js` and published in the contract manifest as `evidenceIntake.poseContract`): orientation, contraction, and pose variant are not independent, only seven combinations exist, and Double Biceps is a Rear, Flexed pose. Intake refuses any other combination with `PHOTO_IDENTITY_NON_CANONICAL` before media is stored or interpreted, so an identity a client lets the Founder confirm can never later become "pose still to choose". The session Goal relationship is resolved deterministically from the scheduled occurrence's current owner (Goals completed before the evidence date never compete, and historical links never override ownership); only a genuinely undecidable case stays `needs_review`. `evidence-review.commit.v1` runs the photo readiness gate (every pose canonical, session details decided) before its confirmation receipt is recorded and returns a typed 400 (`PHOTO_POSE_UNRESOLVED`, `PHOTO_SESSION_DETAILS_UNRESOLVED`) instead of an acknowledgement when the review cannot proceed.
