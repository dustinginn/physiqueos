...
| --- | --- | --- | --- |
| Production server | `a191c27440f953ad05af8d1a09e9ec90d46cdc95` | `origin/combined-app-platform-cutover` and `origin/claude/server-build33-operating-plan-notifications-training` | `/Users/dustinginn/Developer/PhysiqueOS/server` |
...
- `native-production-read-foundation`: canonical default Mac workspace and
  ...
  `codex/project-cleanup-audit`. This is a *linked* worktree — its `.git` is
  a pointer file into `server/.git/worktrees/native-production-read-foundation`.
- `server`: the **main** Git worktree — it owns the real `.git` directory;
  ...
  `server/.git/worktrees/<name>`. **This is now the current authoritative
  Server development workspace**, checked out on branch
  `claude/build39-weekly-monthly-dismissed-replacement` at exact commit
  `a191c27440f953ad05af8d1a09e9ec90d46cdc95` — the production server
  authority — as of the 2026-09-17 relocation. `node_modules` was carried
  over unchanged (package.json/package-lock.json were byte-identical to the
  prior checkout, confirmed before the switch; `npm ls --depth=0` and
  `npm run test:foundation` — 36/36 passing — both verified afterward with
  no reinstall). Do not delete this directory or attempt `git worktree move`
  on it as though it were an ordinary linked worktree — Git does not support
  moving the main worktree that way; a branch-checkout swap in place is the
  correct operation, which is what the 2026-09-17 relocation did.
...
The prior nested worktree at
`native-production-read-foundation/.tmp/server-build39-followup` is
**retired** (removed via `git worktree remove` on 2026-09-17, after
confirming clean state and that its commit was independently reachable from
`origin/combined-app-platform-cutover` and
`origin/claude/server-build33-operating-plan-notifications-training`). It no
longer exists and no longer appears in `git worktree list`. Do not recreate
it; `server` is now the obvious, discoverable home for the production
authority. If a future session finds any script or note still pointing at
that nested path, treat it as stale and redirect to `server`.
...
### Deferred Founder decision: stale worktree policy entries

`config/embedded-repository-policy.json` lists three linked worktrees —
...
(confirmed absent from disk and from `git worktree list`, both on
2026-09-17 and again after the Server relocation). The policy entries
predate the 2026-09-17 cleanup (last touched 2026-08-16) and remain
untouched. `scripts/auditEmbeddedRepositories.mjs` only matches policy
entries against paths it actually discovers on disk, so a dangling entry for
a path that doesn't exist is inert — it causes no audit failure and no false
pass, just a stale registry entry. `HISTORICAL — pending Founder decision`:
confirm whether these three were already legitimately retired (the
`windows-deploy-9c1aa80c` entry itself says it "may be retired via `git
worktree remove` once the migration/recovery checkpoint is accepted," and
its cited commit `9c1aa80c…` is an old production authority, not the
current one) or whether they still exist elsewhere (e.g. the PC) and these
Mac policy entries are simply carried-over metadata. Do not delete or edit
these policy entries without that confirmation — the two `264a6306`-suffixed
entries are marked protected incident evidence. This is unrelated to and
unaffected by the Server workspace relocation above.
