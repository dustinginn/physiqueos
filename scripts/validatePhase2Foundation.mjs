...
const isolatedDist = path.join(root, ".next-phase2-validation");
if (path.dirname(isolatedDist) !== path.resolve(root) || path.basename(isolatedDist) !== ".next-phase2-validation") {
  throw new Error("The Phase 2 isolated validation build path escaped the repository root.");
}
...
const steps = [
  ...
  ["targeted lint", process.execPath, [eslint,
    ...
    "scripts/validatePhase2Foundation.mjs", "scripts/validatePhase2Postgres.mjs", "scripts/runFoundationWeb.mjs", "scripts/runFoundationWorker.mjs",
    "scripts/foundationBackup.mjs", "scripts/provisionPhase2Spaces.mjs", "scripts/validatePhase2ProviderStaging.mjs",
    "scripts/validateDeployedPhase2Worker.mjs", "scripts/cleanupPhase2ProviderFixtures.mjs", "scripts/validatePhase2ProviderRestore.mjs",
    "infra/digitalocean/renderAppSpec.mjs", "vitest.foundation.config.js", "vitest.phase2.config.js",
  ]],
  ["production build", process.execPath, [next, "build"]],
  ["diff check", "git", ["diff", "--check"]],
];

const childEnv = { ...process.env, NODE_OPTIONS: "--max-old-space-size=1536", PHYSIQUEOS_BUILD_DIST_DIR: ".next-phase2-validation" };
try {
