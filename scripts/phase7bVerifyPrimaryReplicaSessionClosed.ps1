...
try {
  if ($AttemptId -notmatch '^phase7b-wp2-[0-9a-f]{32}$' -or $ReceiverUncRoot -notmatch '^\\(?<server>[^\]+)\(?<share>[^\]+)$') { throw 'PHASE7B_WP2_PRIMARY_REPLICA_SESSION_ARGUMENT_FAIL' }
  $server = [string]$Matches.server; $share = [string]$Matches.share
  $expectedShare = "P7B$($AttemptId.Substring($AttemptId.Length - 8))`$"
  if ($server -ne 'LAPTOP-4G5U0U2R' -or $share -ne $expectedShare) { throw 'PHASE7B_WP2_PRIMARY_REPLICA_SESSION_IDENTITY_FAIL' }
