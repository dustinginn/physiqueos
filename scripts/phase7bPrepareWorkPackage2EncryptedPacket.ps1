...
param(
  ...
  [Parameter()][string]$ExpectedAgeKeygenSha256,
  [Parameter()][string]$AgeRecipient,
...
if ($Operation -eq "Inspect") {
  [ordered]@{
    ...
    clipboardSecretPermitted = $true
    clipboardReadOrPersistencePerformedByTooling = $false
    maskedPasteEntryRequired = $true
    ageEncryptionMode = 'native-recipient-v1'
    nativeIdentityConfirmationRequired = $true
    identityInputMode = 'stdin'
    agePluginRequired = $false
    autogeneratedAgePassphrasePermitted = $false
...
try {
  foreach ($requiredValue in @($AttemptId, $AuthorizationPath, $ExpectedAuthorizationSha256, $CapturePlanPath,
      ...
      $LocalOutputDirectory, $ReplicaDirectory, $AgeExePath, $ExpectedAgeExeSha256, $AgeKeygenPath,
      $ExpectedAgeKeygenSha256, $AgeRecipient)) {
  ...
  if ($ExpectedInvocationContractSha256 -notmatch '^[0-9a-fA-F]{64}$' -or $ExpectedStage3LauncherSha256 -notmatch '^[0-9a-fA-F]{64}$' -or
      $ExpectedCapturePlanSha256 -notmatch '^[0-9a-fA-F]{64}$' -or $ExpectedAgeExeSha256 -notmatch '^[0-9a-fA-F]{64}$' -or
      $ExpectedAgeKeygenSha256 -notmatch '^[0-9a-fA-F]{64}$' -or $AgeRecipient -cnotmatch '^age1[023456789acdefghjklmnpqrstuvwxyz]{58}$' -or
      $null -eq $AgeIdentity -or $AgeIdentity.Length -ne 74) { throw "PHASE7B_WP2_CAPTURE_HASH_ARGUMENT_INVALID" }
  ...
  if (-not (Test-Path -LiteralPath $AgeKeygenPath -PathType Leaf) -or
      (Get-Phase7BSha256 -LiteralPath $AgeKeygenPath) -ne $ExpectedAgeKeygenSha256.ToLowerInvariant()) { throw "PHASE7B_WP2_AGE_KEYGEN_IDENTITY_MISMATCH" }
  ...
  $identityVerification = Assert-Phase7BAgeIdentityRecipient -AgeKeygenPath $AgeKeygenPath -Identity $AgeIdentity -ExpectedAgeRecipient $AgeRecipient
  if (-not [bool]$identityVerification.pass) { throw 'PHASE7B_WP2_AGE_IDENTITY_RECIPIENT_MISMATCH' }
  ...
  $authorization = Assert-Phase7BWorkPackage2CaptureAuthorization -LiteralPath $AuthorizationPath -ExpectedSha256 $ExpectedAuthorizationSha256 `
    ...
    -ExpectedAgeExeSha256 $ExpectedAgeExeSha256.ToLowerInvariant() -ExpectedAgeKeygenSha256 $ExpectedAgeKeygenSha256.ToLowerInvariant() `
    -ExpectedAgeRecipient $AgeRecipient -ExpectedQuiescenceEvidenceSha256 ([string]$authorizationPreview.quiescenceEvidenceSha256)
  ...
  if ([string]$authorization.ageExePathSha256 -ne $agePathSha256 -or [string]$authorization.ageKeygenPathSha256 -ne $ageKeygenPathSha256) {
    throw 'PHASE7B_WP2_AGE_PATH_IDENTITY_MISMATCH'
  }
  ...
  $stage = "native-recipient-age-encryption"
  $localPacketPath = Join-Path $localAttemptRoot "$AttemptId$($contract.packetExtension)"
  $ageEncryption = Invoke-Phase7BAgeNativeRecipientEncryption -AgeExePath $AgeExePath -InputPath $plainZipPath -OutputPath $localPacketPath -AgeRecipient $AgeRecipient
  if (-not [bool]$ageEncryption.pass -or [string]$ageEncryption.classification -cne 'PHASE7B_WP2_AGE_NATIVE_RECIPIENT_ENCRYPT_PASS' -or [bool]$ageEncryption.secretInputUsed) {
    throw "PHASE7B_WP2_AGE_NATIVE_RECIPIENT_ENCRYPT_FAILED"
  }
  ...
  $stage = 'native-identity-decrypt-to-hash-verification'
  $roundTrip = Invoke-Phase7BAgeNativeIdentityDecryptionToHash -AgeExePath $AgeExePath -CiphertextPath $localPacketPath -Identity $AgeIdentity
  ...
  $descriptor = [ordered]@{
    ...
    ageKeygenSha256 = $ExpectedAgeKeygenSha256.ToLowerInvariant()
    ageKeygenVersion = [string]$ageKeygenVersion.normalizedVersion
