...
Import-Module (Join-Path $PSScriptRoot "phase7bWindowsAgePassphraseBridge.psm1") -Force
...
if ($Operation -eq "Inspect") {
  [ordered]@{
    ...
    explicitPassphraseConfirmationRequired = $true
...
try {
  ...
  Write-Host "PHASE7B_WP2_ENTER_PASSPHRASE_IN_MASKED_WINDOWS_DIALOG"
  $ageBridge = Invoke-Phase7BAgeEncryptionWithSecureWindowsInput -AgeExePath $AgeExePath -InputPath $plainZipPath -OutputPath $localPacketPath
  if (-not [bool]$ageBridge.pass -or [string]$ageBridge.classification -cne 'PHASE7B_WP2_AGE_SECURE_INPUT_BRIDGE_PASS' -or
      -not [bool]$ageBridge.explicitPassphraseConfirmationSupplied -or [bool]$ageBridge.autogeneratedPassphrasePathReachable) {
    if ([string]$ageBridge.safeErrorCode -match '^PHASE7B_') { throw [string]$ageBridge.safeErrorCode }
    throw "PHASE7B_WP2_AGE_SECURE_INPUT_BRIDGE_FAILED"
  }
