...
function Read-Phase7BWP2CPreparationContinuation {
  ...
  Assert-Phase7BWP2C ((Get-Phase7BWP2CVmxIdentity $vmx).sha256 -ceq $c.vm.configSha256 -and (Get-Phase7BWP2CExpectedGuestIdentity $vmx) -ceq $c.vm.guestIdentitySha256 -and $c.vm.guestIdentitySha256 -ceq $s.expectedGuestIdentitySha256) 'CONTINUATION_VM_BINDING'
  ...
  [pscustomobject]@{document=$c;identity=Get-Phase7BWP2CIdentity $Path;root=$root;settings=$settings;tooling=$tool}
}
