// Bundles the repair entry (and the exact domain code it depends on) into the single
// file the accepted console runner transports. Usage:
//   node scripts/operations/buildPhotoSessionReviewRepairPayload.mjs --sha <40-hex> \
...
export async function buildPhotoSessionReviewRepairPayload({ sha, mode = "dry-run", authorizationReference = "", marker } = {}) {
  ...
  const successMarker = marker ?? `PHYSIQUEOS_PHOTO_REVIEW_REPAIR_${mode === "apply" ? "APPLY" : "DRYRUN"}_SUCCESS_${randomBytes(4).toString("hex")}`;
  const result = await build({
    entryPoints: [path.join(root, "scripts/operations/photoSessionReviewRepair.entry.mjs")],
    bundle: true,
    write: false,
    format: "esm",
    platform: "node",
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  const { code, marker } = await buildPhotoSessionReviewRepairPayload({ sha: args.sha, mode: args.mode, authorizationReference: args["authorization-ref"] });
