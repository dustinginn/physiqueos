// Bounded HealthKit Workout link reassessment (dry-run | apply).
// Dry-run is REPEATABLE READ READ ONLY. Apply runs under the per-owner
// advisory lock, requires fresh dry-run facts plus an authorization reference,
// and can create only a quarantined candidate, update one workout assessment,
// and write one audit row. It never confirms or graduates evidence.
import { createRequire } from "node:module";
import { runHealthKitWorkoutLinkReassessment } from "../../src/platform/operations/HealthKitWorkoutLinkReassessmentRunner.js";
...
const DATE = typeof __DATE__ === "undefined" ? "" : __DATE__;
...
const MARKER = typeof __MARKER__ === "undefined" ? "PHYSIQUEOS_HEALTHKIT_LINK_REASSESSMENT_SUCCESS" : __MARKER__;
...
const watchdog = setTimeout(() => stop("REASSESSMENT_TIMEOUT", 3), 90_000);

function stop(code, status = 1) {
  process.stdout.write(`PHYSIQUEOS_HEALTHKIT_LINK_REASSESSMENT_FAILED:${code}\n`);
  process.exit(status);
}
const sanitizedCode = (error) => (/^[A-Za-z0-9_]{3,40}$/.test(String(error?.code ?? "")) ? String(error.code) : "REASSESSMENT_ERROR");

if (!["dry-run", "apply"].includes(MODE)) stop("MODE_INVALID");
if (!/^\d{4}-\d{2}-\d{2}$/.test(DATE)) stop("DATE_INVALID");
...
const pool = new pg.Pool({
  ...
  max: 1,
  application_name: "physiqueos-healthkit-link-reassessment",
...
try {
  ...
  result = await runHealthKitWorkoutLinkReassessment({
    records,
    authorization: { ownerUserId: OWNER, localDate: DATE, authorizationReference: AUTHORIZATION_REFERENCE },
    apply,
    expected,
  });
...
if (failure || !result) stop(failure ?? "REASSESSMENT_INCOMPLETE");
process.stdout.write(`PHYSIQUEOS_HEALTHKIT_LINK_REASSESSMENT_JSON:${JSON.stringify({ mode: MODE, localDate: DATE, authorizationReference: AUTHORIZATION_REFERENCE || null, ...result })}\n`);
const acceptable = MODE === "apply" ? ["applied", "already_reassessed"] : ["dry_run", "already_reassessed"];
