// Bounded administrative Training historical repair (Sep 13 correction, retroactive events).
//
// Transported into the existing App Platform `web` component by the accepted console runner
// (bundled to one file by buildTrainingAuthorityRepairPayload.mjs). It follows the
// production-operations contract: identity gates before any database access, one bounded
// connection, owner scoping, explicit transaction fencing, no secrets printed, and a success
// marker only after a clean COMMIT (apply) or ROLLBACK (dry-run).
//
...
//            transaction: owner runtime lock first, read, re-plan, drift check, new-row writes
//            only, in-transaction verification, COMMIT (any invariant failure rolls back).
//
// Executing this is a separate, explicitly authorized act. Nothing here runs it.
import { createRequire } from "node:module";
import { runTrainingAuthorityRepair } from "../../src/platform/operations/TrainingAuthorityRepairRunner.js";
import { createPhase4CanonicalRecordStore } from "../../src/platform/database/Phase4CanonicalRecordStore.js";
import { TRAINING_AUTHORITY_REPAIR_AUTHORIZATION as AUTHORIZATION } from "../../src/platform/operations/trainingAuthorityRepairAuthorization.js";
...
const PHASE = typeof __PHASE__ === "undefined" ? "" : __PHASE__;
...
const MARKER = typeof __MARKER__ === "undefined" ? "PHYSIQUEOS_TRAINING_REPAIR_SUCCESS" : __MARKER__;
...
function stop(code, status = 1) {
  process.stdout.write(`PHYSIQUEOS_TRAINING_REPAIR_FAILED:${code}\n`);
  process.exit(status);
}
...
if (!["sep13_correction", "retroactive_events"].includes(PHASE)) stop("PHASE_INVALID");
...
if (EXPECTED_GIT_SHA !== AUTHORIZATION.expectedProductionSha) stop("AUTHORIZED_SHA_MISMATCH");
...
const pool = new pg.Pool({
  ...
  max: 1,
  application_name: "physiqueos-training-authority-repair",
...
try {
  ...
  result = await runTrainingAuthorityRepair({ records, authorization: AUTHORIZATION, phase: PHASE, apply, expected });
...
process.stdout.write(`PHYSIQUEOS_TRAINING_REPAIR_JSON:${JSON.stringify({ mode: MODE, phase: PHASE, authorizationReference: AUTHORIZATION_REFERENCE || null, ...result })}\n`);
