// Zero-write HealthKit canonical acceptance audit (proving window).
//
...
import { summarizeHealthKitCanonicalAcceptance } from "../../src/platform/operations/HealthKitCanonicalAcceptanceAudit.js";
...
const MARKER = typeof __MARKER__ === "undefined" ? "PHYSIQUEOS_HEALTHKIT_ACCEPTANCE_AUDIT_SUCCESS" : __MARKER__;
...
const STRATEGIC_TABLES = Object.freeze([
  ...
  "canonical_plan_records", "canonical_protocol_records", "canonical_evidence_records",
]);
const watchdog = setTimeout(() => stop("AUDIT_TIMEOUT", 3), 150_000);

function stop(code, status = 1) {
  process.stdout.write(`PHYSIQUEOS_HEALTHKIT_ACCEPTANCE_AUDIT_FAILED:${code}\n`);
  process.exit(status);
}
...
try {
  ...
  const [policyRecord, observations, canonicalDays, canonicalEvidenceObjects] = await Promise.all([
    records.get({ ownerUserId: OWNER, collection: "healthKitConfiguration", recordId: "healthkit_canonical_daily_activation_policy" }),
    records.list({ ownerUserId: OWNER, collection: "healthKitObservations" }),
    records.list({ ownerUserId: OWNER, collection: "healthKitCanonicalDays" }),
    records.list({ ownerUserId: OWNER, collection: "canonicalEvidenceObjects" }),
  ]);
  const summary = summarizeHealthKitCanonicalAcceptance({
    policyRecord, observations, canonicalDays, canonicalEvidenceObjects,
...
process.stdout.write(`PHYSIQUEOS_HEALTHKIT_ACCEPTANCE_AUDIT_JSON:${JSON.stringify(report)}\n`);
