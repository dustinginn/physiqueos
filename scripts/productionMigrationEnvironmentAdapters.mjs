...
} from "../src/platform/migration/readModelParityComparison.js";

register("./sourceModuleResolutionHook.mjs", import.meta.url);

// Re-exported for backward compatibility: this module used to define these directly, and
// scripts/productionMigrationEnvironmentAdapters.test.js and other callers still import them from
// here. The real implementation now lives in src/platform/migration/readModelParityComparison.js
// so src/-owned production code (including the combined-cutover parity adapter) can reuse it
// without importing a scripts/*.mjs entry point.
export { semanticReadModelProjection, readModelsSemanticallyEqual, computeBoundedSemanticDifference, compareRepresentativeReads };
