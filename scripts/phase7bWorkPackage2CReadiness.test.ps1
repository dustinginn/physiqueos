...
try {
  ...
  foreach($field in @('memsize','ethernet0.startconnected','sata0:0.filename')){$bad=$vmx.Clone();$bad[$field]='changed';Assert-True ((Get-Phase7BWP2CVmxIdentity $bad).sha256 -cne $vmIdentity.sha256) "VM config binds $field"}
