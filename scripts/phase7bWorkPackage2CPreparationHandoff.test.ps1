...
try {
  ...
  Assert-True (@($plan.toolingManifest.files).Count -eq 12) 'fixed tooling closure remains 12 PS plus two binaries plus manifest'
  ...
  if($AgeExePath -and $AgeKeygenPath){
    ...
    Assert-True (@(Get-ChildItem -LiteralPath $tools).Count -eq 15) 'actual tooling producer still exactly 15 files'
