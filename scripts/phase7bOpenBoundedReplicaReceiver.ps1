...
param(
  ...
  [Parameter()][int64]$ExpectedPacketBytes,
...
try {
  if ($AttemptId -notmatch '^phase7b-wp2-[0-9a-f]{32}$' -or $ExpectedPacketFileName -ne "$AttemptId.zip.age" -or
      $ExpectedPacketBytes -le 0 -or $AuthorizationAcknowledgement -ne 'WP2B_CAPTURE_EPHEMERAL_REPLICA_OPEN_EXACTLY_ONCE') { throw 'PHASE7B_WP2_BOUNDED_REPLICA_ARGUMENT_OR_AUTHORIZATION_FAIL' }
  ...
  if (-not (Test-Phase7BBoundedReplicaDestinationEvidence -Evidence $evidence -RequiredBytes $ExpectedPacketBytes).pass) { throw 'PHASE7B_WP2_BOUNDED_REPLICA_DESTINATION_FAIL' }
  ...
  [ordered]@{ classification = 'PHASE7B_WP2_BOUNDED_REPLICA_EPHEMERAL_RECEIVER_OPEN'; pass = $true; attemptId = $AttemptId; computerName = $contract.acceptedComputerName; hostIdentitySha256 = $hostSha; diskIdentitySha256 = $diskSha; shareName = $shareName; uncReplicaRoot = "\\$($contract.acceptedComputerName)\$shareName"; packetFileName = $ExpectedPacketFileName; packetBytes = $ExpectedPacketBytes; existingAccountUsed = $true; credentialsPersisted = $false; teardownRequired = $true; automaticRetryAllowed = $false } | ConvertTo-Json -Depth 4
} catch {
