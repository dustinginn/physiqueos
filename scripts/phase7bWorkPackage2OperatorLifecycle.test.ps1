...
try {
  ...
  $tooling='a'*40;$invocation='6'*64;$stage3='7'*64;$plan='b'*64;$inventory='c'*64;$selection='d'*64;$source='e'*64;$runtime='f'*64;$agePath='1'*64;$age='2'*64;$ageKeygenPath='8'*64;$ageKeygen='9'*64;$recipient='age1'+('q'*58);$local='3'*64;$replica='4'*64;$quiescenceSha='5'*64
  $unc='\\LAPTOP-4G5UOU2R\P7Baaaaaaaa$';$issued=[DateTime]::UtcNow.AddMinutes(-1)
  $document=New-Phase7BWorkPackage2CaptureAuthorizationDocument -AuthorizationId $authorizationId -AttemptId $attempt -ToolingCommit $tooling -InvocationContractSha256 $invocation -Stage3LauncherSha256 $stage3 -CapturePlanSha256 $plan -CapturePlanFileName "$attempt-refresh-$('2'*32)-capture-plan.json" -InventorySha256 $inventory -SelectionSha256 $selection -SelectionFileName "$attempt-refresh-$('2'*32)-selection.json" -SourceRootSha256 $source -RuntimeRevision 142 -RuntimeSha256 $runtime -AgeExePathSha256 $agePath -AgeExeSha256 $age -AgeVersion '1.3.1' -AgeKeygenPathSha256 $ageKeygenPath -AgeKeygenSha256 $ageKeygen -AgeKeygenVersion '1.3.1' -AgeRecipient $recipient -LocalOutputRootSha256 $local -ReplicaRootSha256 $replica -ReplicaUncRoot $unc -QuiescenceEvidenceSha256 $quiescenceSha -QuiescenceEvidenceFileName "phase7b-wp2b-quiescence-$('1'*32).json" -QuiescenceEvidenceToolingCommit $tooling -ConsumptionMarkerFileName "$authorizationId.used.json" -IssuedAt $issued -ExpiresAt $issued.AddHours(1)
  Assert-True ($document.invocationContractSha256 -ceq $invocation -and $document.stage3LauncherSha256 -ceq $stage3 -and $document.ageRecipient -ceq $recipient -and $document.nativeRecipientRequired -and -not $document.agePluginRequired -and $document.decryptRoundTripRequired) 'authorization explicitly binds invocation Stage3 native recipient and decrypt round trip'
  ...
  $tooLongParameters=@{AuthorizationId=$authorizationId;AttemptId=$attempt;ToolingCommit=$tooling;InvocationContractSha256=$invocation;Stage3LauncherSha256=$stage3;CapturePlanSha256=$plan;CapturePlanFileName="$attempt-refresh-$('2'*32)-capture-plan.json";InventorySha256=$inventory;SelectionSha256=$selection;SelectionFileName="$attempt-refresh-$('2'*32)-selection.json";SourceRootSha256=$source;RuntimeRevision=142;RuntimeSha256=$runtime;AgeExePathSha256=$agePath;AgeExeSha256=$age;AgeVersion='1.3.1';AgeKeygenPathSha256=$ageKeygenPath;AgeKeygenSha256=$ageKeygen;AgeKeygenVersion='1.3.1';AgeRecipient=$recipient;LocalOutputRootSha256=$local;ReplicaRootSha256=$replica;ReplicaUncRoot=$unc;QuiescenceEvidenceSha256=$quiescenceSha;QuiescenceEvidenceFileName="phase7b-wp2b-quiescence-$('1'*32).json";QuiescenceEvidenceToolingCommit=$tooling;ConsumptionMarkerFileName="$authorizationId.used.json";IssuedAt=$issued;ExpiresAt=$issued.AddHours(25)}
  ...
  $accepted=Assert-Phase7BWorkPackage2CaptureAuthorization -LiteralPath $authPath -ExpectedSha256 $authHash -ExpectedAttemptId $attempt -ExpectedToolingCommit $tooling -ExpectedInvocationContractSha256 $invocation -ExpectedStage3LauncherSha256 $stage3 -ExpectedInventorySha256 $inventory -ExpectedSourceRootSha256 $source -ExpectedCapturePlanSha256 $plan -ExpectedLocalOutputRootSha256 $local -ExpectedReplicaRootSha256 $replica -ExpectedAgeExeSha256 $age -ExpectedAgeKeygenSha256 $ageKeygen -ExpectedAgeRecipient $recipient -ExpectedQuiescenceEvidenceSha256 $quiescenceSha
  Assert-True ($accepted.authorizationId -eq $authorizationId) 'exact capture authorization accepted'
  foreach($case in @(@{p='ExpectedToolingCommit';v=('0'*40)},@{p='ExpectedInvocationContractSha256';v=('0'*64)},@{p='ExpectedStage3LauncherSha256';v=('0'*64)},@{p='ExpectedInventorySha256';v=('0'*64)},@{p='ExpectedCapturePlanSha256';v=('0'*64)},@{p='ExpectedAgeExeSha256';v=('0'*64)},@{p='ExpectedAgeKeygenSha256';v=('0'*64)},@{p='ExpectedAgeRecipient';v=('age1'+('p'*58))},@{p='ExpectedQuiescenceEvidenceSha256';v=('0'*64)})){
    $parameters=@{LiteralPath=$authPath;ExpectedSha256=$authHash;ExpectedAttemptId=$attempt;ExpectedToolingCommit=$tooling;ExpectedInvocationContractSha256=$invocation;ExpectedStage3LauncherSha256=$stage3;ExpectedInventorySha256=$inventory;ExpectedSourceRootSha256=$source;ExpectedCapturePlanSha256=$plan;ExpectedLocalOutputRootSha256=$local;ExpectedReplicaRootSha256=$replica;ExpectedAgeExeSha256=$age;ExpectedAgeKeygenSha256=$ageKeygen;ExpectedAgeRecipient=$recipient;ExpectedQuiescenceEvidenceSha256=$quiescenceSha};$parameters[$case.p]=$case.v
    Assert-Throws { Assert-Phase7BWorkPackage2CaptureAuthorization @parameters } 'MISMATCH' "capture authorization rejects $($case.p)"
  }
  foreach($property in @('nativeRecipientRequired','decryptRoundTripRequired')){
    $bad=[ordered]@{};foreach($key in $document.Keys){$bad[$key]=$document[$key]};$bad[$property]=$false
    $badPath=Join-Path $testRoot "$property-false.json";Write-Canonical $badPath $bad
    $parameters=@{LiteralPath=$badPath;ExpectedSha256=(Get-Phase7BSha256 -LiteralPath $badPath);ExpectedAttemptId=$attempt;ExpectedToolingCommit=$tooling;ExpectedInvocationContractSha256=$invocation;ExpectedStage3LauncherSha256=$stage3;ExpectedInventorySha256=$inventory;ExpectedSourceRootSha256=$source;ExpectedCapturePlanSha256=$plan;ExpectedLocalOutputRootSha256=$local;ExpectedReplicaRootSha256=$replica;ExpectedAgeExeSha256=$age;ExpectedAgeKeygenSha256=$ageKeygen;ExpectedAgeRecipient=$recipient;ExpectedQuiescenceEvidenceSha256=$quiescenceSha}
    Assert-Throws { Assert-Phase7BWorkPackage2CaptureAuthorization @parameters } 'MISMATCH' "authorization rejects $property false"
    [void]$bad.Remove($property);$missingPath=Join-Path $testRoot "$property-missing.json";Write-Canonical $missingPath $bad
    $parameters.LiteralPath=$missingPath;$parameters.ExpectedSha256=Get-Phase7BSha256 -LiteralPath $missingPath
    Assert-Throws { Assert-Phase7BWorkPackage2CaptureAuthorization @parameters } 'MISMATCH' "authorization rejects missing $property"
  }
  foreach($change in @(
    @{name='ageEncryptionMode';value='passphrase'},@{name='ageRecipient';value=('age1'+('p'*58))},
    @{name='ageIdentityInputMode';value='console'},@{name='agePluginRequired';value=$true},
    @{name='ageVersion';value='1.2.0'},@{name='ageKeygenSha256';value=('0'*64)},
    @{name='ageKeygenVersion';value='1.2.0'})) {
    $bad=[ordered]@{};foreach($key in $document.Keys){$bad[$key]=$document[$key]};$bad[$change.name]=$change.value
    $badPath=Join-Path $testRoot "$($change.name)-wrong.json";Write-Canonical $badPath $bad
    $parameters=@{LiteralPath=$badPath;ExpectedSha256=(Get-Phase7BSha256 -LiteralPath $badPath);ExpectedAttemptId=$attempt;ExpectedToolingCommit=$tooling;ExpectedInvocationContractSha256=$invocation;ExpectedStage3LauncherSha256=$stage3;ExpectedInventorySha256=$inventory;ExpectedSourceRootSha256=$source;ExpectedCapturePlanSha256=$plan;ExpectedLocalOutputRootSha256=$local;ExpectedReplicaRootSha256=$replica;ExpectedAgeExeSha256=$age;ExpectedAgeKeygenSha256=$ageKeygen;ExpectedAgeRecipient=$recipient;ExpectedQuiescenceEvidenceSha256=$quiescenceSha}
    Assert-Throws { Assert-Phase7BWorkPackage2CaptureAuthorization @parameters } 'MISMATCH' "authorization rejects wrong $($change.name)"
  }
  ...
  Assert-Throws { Assert-Phase7BWorkPackage2CaptureAuthorization -LiteralPath $authPath -ExpectedSha256 $authHash -ExpectedAttemptId $attempt -ExpectedToolingCommit $tooling -ExpectedInvocationContractSha256 $invocation -ExpectedStage3LauncherSha256 $stage3 -ExpectedInventorySha256 $inventory -ExpectedSourceRootSha256 $source -ExpectedCapturePlanSha256 $plan -ExpectedLocalOutputRootSha256 $local -ExpectedReplicaRootSha256 $replica -ExpectedAgeExeSha256 $age -ExpectedAgeKeygenSha256 $ageKeygen -ExpectedAgeRecipient $recipient -ExpectedQuiescenceEvidenceSha256 $quiescenceSha } 'ALREADY_USED' 'authorization reuse rejected'
...
"@
  ...
  $finalInvocationPath=Join-Path $testRoot 'final-invocation.json'
  $invocationDocument=New-Phase7BWorkPackage2InvocationContractDocument -AttemptId $attempt -ToolingCommit $tooling `
    -ApplicationCommit (Get-Phase7BWorkPackage2Contract).applicationCommit -Artifacts @([pscustomobject]@{relativePath='scripts/phase7bRunWorkPackage2Stage3.ps1';sha256=$stage3;bytes=1}) `
    -AgeRecipient $recipient -AgeExePathSha256 $agePath -AgeExeSha256 $age -AgeVersion '1.3.1' `
    -AgeKeygenPathSha256 $ageKeygenPath -AgeKeygenSha256 $ageKeygen -AgeKeygenVersion '1.3.1'
  Write-Canonical $finalInvocationPath $invocationDocument;$invocation=Get-Phase7BSha256 -LiteralPath $finalInvocationPath
  $finalQuiescencePath=Join-Path $testRoot "phase7b-wp2b-quiescence-$('1'*32).json"
  Write-Canonical $finalQuiescencePath (New-Quiescence $tooling);$quiescenceSha=Get-Phase7BSha256 -LiteralPath $finalQuiescencePath
  $finalExternalInputs=@{InvocationContractPath=$finalInvocationPath;ExpectedPacketSha256=$packetSha;ExpectedPacketBytes=$packetBytes}
  $finalAuthorization=New-Phase7BWorkPackage2CaptureAuthorizationDocument -AuthorizationId $finalAuthorizationId -AttemptId $attempt -ToolingCommit $tooling -InvocationContractSha256 $invocation -Stage3LauncherSha256 $stage3 -CapturePlanSha256 $plan -CapturePlanFileName "$attempt-refresh-$('2'*32)-capture-plan.json" -InventorySha256 $inventory -SelectionSha256 $selection -SelectionFileName "$attempt-refresh-$('2'*32)-selection.json" -SourceRootSha256 $source -RuntimeRevision 142 -RuntimeSha256 $runtime -AgeExePathSha256 $agePath -AgeExeSha256 $age -AgeVersion '1.3.1' -AgeKeygenPathSha256 $ageKeygenPath -AgeKeygenSha256 $ageKeygen -AgeKeygenVersion '1.3.1' -AgeRecipient $recipient -LocalOutputRootSha256 $local -ReplicaRootSha256 $replica -ReplicaUncRoot $unc -QuiescenceEvidenceSha256 $quiescenceSha -QuiescenceEvidenceFileName "phase7b-wp2b-quiescence-$('1'*32).json" -QuiescenceEvidenceToolingCommit $tooling -ConsumptionMarkerFileName "$finalAuthorizationId.used.json" -IssuedAt ([DateTime]::UtcNow.AddMinutes(-1)) -ExpiresAt ([DateTime]::UtcNow.AddHours(1))
  ...
  $pendingPath=Join-Path $testRoot 'pending.json';Write-Canonical $pendingPath (New-Phase7BSyntheticPendingDescriptor -Authorization $finalAuthorization -PacketSha256 $packetSha -PacketBytes $packetBytes)
  $finalPath=Join-Path $testRoot 'final.json';$final=@(& (Join-Path $PSScriptRoot 'phase7bFinalizeBoundedReplicaDescriptor.ps1') @finalExternalInputs -AttemptId $attempt -ExpectedInvocationContractSha256 $invocation -ExpectedStage3LauncherSha256 $stage3 -PendingDescriptorPath $pendingPath -ExpectedPendingDescriptorSha256 (Get-Phase7BSha256 -LiteralPath $pendingPath) -ReplicaReceiptPath $receiptPath -ExpectedReplicaReceiptSha256 $receiptPersisted.sha256 -PrimaryTeardownEvidencePath $teardownPath -ExpectedPrimaryTeardownEvidenceSha256 $teardownPersisted.sha256 -CaptureAuthorizationPath $finalAuthorizationPath -ExpectedCaptureAuthorizationSha256 $finalAuthorizationSha -ExpectedToolingCommit $tooling -AuthorizationAcknowledgement 'WP2B_CAPTURE_FINALIZE_INDEPENDENT_REPLICA_EXACTLY_ONCE' -OutputPath $finalPath)-join [Environment]::NewLine|ConvertFrom-Json
  ...
  $failure=@(& (Join-Path $PSScriptRoot 'phase7bFinalizeBoundedReplicaDescriptor.ps1') @finalExternalInputs -AttemptId $attempt -ExpectedInvocationContractSha256 $invocation -ExpectedStage3LauncherSha256 $stage3 -PendingDescriptorPath $pendingPath -ExpectedPendingDescriptorSha256 (Get-Phase7BSha256 -LiteralPath $pendingPath) -ReplicaReceiptPath $wrongReceipt -ExpectedReplicaReceiptSha256 $receiptPersisted.sha256 -PrimaryTeardownEvidencePath $teardownPath -ExpectedPrimaryTeardownEvidenceSha256 $teardownPersisted.sha256 -CaptureAuthorizationPath $finalAuthorizationPath -ExpectedCaptureAuthorizationSha256 $finalAuthorizationSha -ExpectedToolingCommit $tooling -AuthorizationAcknowledgement 'WP2B_CAPTURE_FINALIZE_INDEPENDENT_REPLICA_EXACTLY_ONCE' -OutputPath $wrongFinal)-join [Environment]::NewLine|ConvertFrom-Json
  ...
  $ageBridgeSource=Get-Content -Raw (Join-Path $PSScriptRoot 'phase7bWindowsAgeIdentityBridge.psm1')
  ...
  Assert-True (-not $captureSource.Contains('Use-Phase7BWorkPackage2CaptureAuthorization') -and
    $captureSource.Contains('Invoke-Phase7BAgeNativeIdentityDecryptionToHash')) 'Stage3 verifies decrypt round trip and leaves authorization unconsumed'
  ...
  Assert-True ($captureSource.Contains('Invoke-Phase7BAgeNativeRecipientEncryption') -and -not $captureSource.Contains('& $AgeExePath -p')) 'capture uses only native public-recipient encryption'
  Assert-True ($ageBridgeSource.Contains("Arguments = '-y'") -and $ageBridgeSource.Contains("Arguments = '--decrypt -i -") -and
    -not $ageBridgeSource.Contains('WriteConsoleInputW')) 'age identity bridge derives and decrypts through redirected stdin without ConsoleHost injection'
  Assert-True ($ageBridgeSource.Contains('UseSystemPasswordChar = $true') -and $ageBridgeSource.Contains('ShortcutsEnabled = $true') -and
    $ageBridgeSource.Contains('PHASE7B_WP2_AGE_IDENTITY_CONFIRMATION_FAIL')) 'age bridge provides masked paste entry with explicit native identity confirmation'
