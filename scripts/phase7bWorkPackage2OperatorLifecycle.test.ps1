...
try {
  ...
  $tooling='a'*40;$invocation='6'*64;$stage3='7'*64;$plan='b'*64;$inventory='c'*64;$selection='d'*64;$source='e'*64;$runtime='f'*64;$agePath='1'*64;$age='2'*64;$local='3'*64;$replica='4'*64;$quiescenceSha='5'*64
  ...
  Assert-True ($document.invocationContractSha256 -ceq $invocation -and $document.stage3LauncherSha256 -ceq $stage3 -and $document.securePassphraseBridgeRequired -and $document.decryptRoundTripRequired) 'authorization explicitly binds invocation Stage3 secure bridge and decrypt round trip'
  ...
  $accepted=Assert-Phase7BWorkPackage2CaptureAuthorization -LiteralPath $authPath -ExpectedSha256 $authHash -ExpectedAttemptId $attempt -ExpectedToolingCommit $tooling -ExpectedInvocationContractSha256 $invocation -ExpectedStage3LauncherSha256 $stage3 -ExpectedInventorySha256 $inventory -ExpectedSourceRootSha256 $source -ExpectedCapturePlanSha256 $plan -ExpectedLocalOutputRootSha256 $local -ExpectedReplicaRootSha256 $replica -ExpectedAgeExeSha256 $age -ExpectedQuiescenceEvidenceSha256 $quiescenceSha
  Assert-True ($accepted.authorizationId -eq $authorizationId) 'exact capture authorization accepted'
  foreach($case in @(@{p='ExpectedToolingCommit';v=('0'*40)},@{p='ExpectedInvocationContractSha256';v=('0'*64)},@{p='ExpectedStage3LauncherSha256';v=('0'*64)},@{p='ExpectedInventorySha256';v=('0'*64)},@{p='ExpectedCapturePlanSha256';v=('0'*64)},@{p='ExpectedAgeExeSha256';v=('0'*64)},@{p='ExpectedQuiescenceEvidenceSha256';v=('0'*64)})){
  ...
  foreach($property in @('securePassphraseBridgeRequired','decryptRoundTripRequired')){
