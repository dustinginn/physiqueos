...
"@
  ...
  $pendingPath=Join-Path $testRoot 'pending.json';Write-Canonical $pendingPath ([ordered]@{schemaVersion=1;classification='PHASE7B_WP2_ENCRYPTED_PACKET_REPLICA_COPY_PENDING_INDEPENDENT_READBACK';attemptId=$attempt;invocationContractSha256=$invocation;stage3LauncherSha256=$stage3;ageEncryptionMode='native-recipient-v1';ageRecipient=$recipient;ageIdentityInputMode='stdin';nativeRecipientRequired=$true;agePluginRequired=$false;ageVersion='1.3.1';ageKeygenVersion='1.3.1';decryptRoundTripRequired=$true;packetSha256=$packetSha;packetBytes=$packetBytes;localEncryptedCopyPass=$true;independentEncryptedReplicaPass=$false;plaintextZipSha256=$plainSha;plaintextZipBytes=$plainBytes;decryptedStreamSha256=$plainSha;decryptedStreamBytes=$plainBytes;decryptRoundTripPass=$true;captureAuthorizationId=$finalAuthorizationId;captureAuthorizationSha256=$finalAuthorizationSha;captureAuthorizationToolingCommit=$tooling;sourceInventorySha256=$inventory;sourceRootSha256=$source;capturePlanSha256=$plan;localOutputRootSha256=$local;replicaRootSha256=$replica;ageExeSha256=$age;ageKeygenSha256=$ageKeygen;quiescenceEvidenceSha256=$quiescenceSha})
  $finalPath=Join-Path $testRoot 'final.json';$final=@(& (Join-Path $PSScriptRoot 'phase7bFinalizeBoundedReplicaDescriptor.ps1') -AttemptId $attempt -ExpectedInvocationContractSha256 $invocation -ExpectedStage3LauncherSha256 $stage3 -PendingDescriptorPath $pendingPath -ExpectedPendingDescriptorSha256 (Get-Phase7BSha256 -LiteralPath $pendingPath) -ReplicaReceiptPath $receiptPath -ExpectedReplicaReceiptSha256 $receiptPersisted.sha256 -PrimaryTeardownEvidencePath $teardownPath -ExpectedPrimaryTeardownEvidenceSha256 $teardownPersisted.sha256 -CaptureAuthorizationPath $finalAuthorizationPath -ExpectedCaptureAuthorizationSha256 $finalAuthorizationSha -ExpectedToolingCommit $tooling -AuthorizationAcknowledgement 'WP2B_CAPTURE_FINALIZE_INDEPENDENT_REPLICA_EXACTLY_ONCE' -OutputPath $finalPath)-join [Environment]::NewLine|ConvertFrom-Json
  ...
  $failure=@(& (Join-Path $PSScriptRoot 'phase7bFinalizeBoundedReplicaDescriptor.ps1') -AttemptId $attempt -ExpectedInvocationContractSha256 $invocation -ExpectedStage3LauncherSha256 $stage3 -PendingDescriptorPath $pendingPath -ExpectedPendingDescriptorSha256 (Get-Phase7BSha256 -LiteralPath $pendingPath) -ReplicaReceiptPath $wrongReceipt -ExpectedReplicaReceiptSha256 $receiptPersisted.sha256 -PrimaryTeardownEvidencePath $teardownPath -ExpectedPrimaryTeardownEvidenceSha256 $teardownPersisted.sha256 -CaptureAuthorizationPath $finalAuthorizationPath -ExpectedCaptureAuthorizationSha256 $finalAuthorizationSha -ExpectedToolingCommit $tooling -AuthorizationAcknowledgement 'WP2B_CAPTURE_FINALIZE_INDEPENDENT_REPLICA_EXACTLY_ONCE' -OutputPath $wrongFinal)-join [Environment]::NewLine|ConvertFrom-Json
