...
try {
  if ($AttemptId -notmatch '^phase7b-wp2-[0-9a-f]{32}$' -or $ExpectedEvidenceNonce -notmatch '^[0-9a-f]{32}$' -or
      ...
      (Split-Path -Leaf $OutputPath) -cne "$AttemptId-replica-receipt-$ExpectedEvidenceNonce.json" -or (Test-Path -LiteralPath $OutputPath)) {
