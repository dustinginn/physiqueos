...
try {
  ...
  '{"schemaVersion":1,"revision":7}' | Set-Content -LiteralPath (Join-Path $sourceRoot 'canonical\runtime.json') -Encoding UTF8
...
'@ | Set-Content -LiteralPath $fakeAge -Encoding ASCII
  ...
  Assert-True ($captureResult.pass -and $captureResult.classification -eq 'PHASE7B_WP2_ENCRYPTED_PACKET_AND_REPLICA_PASS') "synthetic capture/encrypt/replicate flow accepted:$($captureResult | ConvertTo-Json -Compress)"
  ...
  Assert-True ($captureResult.replicaPass -and -not $captureResult.plaintextSecretPersisted -and -not $captureResult.automaticRetryAllowed) "capture output proves replica and secret/retry boundary"
  ...
  $trackedPaths = @('phase7bWorkPackage2Contract.psm1','phase7bPlanWorkPackage2Capture.ps1','phase7bPrepareWorkPackage2EncryptedPacket.ps1','phase7bBuildWorkPackage2RestoreIso.ps1','phase7bIsolatedGuestRestoreInterface.ps1','phase7bWorkPackage2Preparation.test.ps1') | ForEach-Object { Join-Path $PSScriptRoot $_ }
