...
'@ | Set-Content -LiteralPath $fakeAge -Encoding ASCII
  ...
  $fakeAgeKeygen = $fakeAge
  $fakeAgeKeygenHash = $fakeAgeHash
  ...
  $finalTooling='a'*40;$finalStage3='7'*64;$finalAuthorizationId='phase7b-wp2b-capture-auth-'+('e'*32)
  $finalInvocationPath=Join-Path $testRoot 'final-invocation.json'
  $finalInvocationDocument=New-Phase7BWorkPackage2InvocationContractDocument -AttemptId $attemptId -ToolingCommit $finalTooling `
    -ApplicationCommit $contract.applicationCommit -Artifacts @([pscustomobject]@{relativePath='scripts/phase7bRunWorkPackage2Stage3.ps1';sha256=$finalStage3;bytes=1}) `
    -AgeRecipient $ageRecipient -AgeExePathSha256 ('e'*64) -AgeExeSha256 $fakeAgeHash -AgeVersion '1.3.1' `
    -AgeKeygenPathSha256 ('8'*64) -AgeKeygenSha256 $fakeAgeKeygenHash -AgeKeygenVersion '1.3.1'
  Write-JsonNoBom $finalInvocationPath $finalInvocationDocument
  $finalInvocation=Get-Phase7BSha256 -LiteralPath $finalInvocationPath
  $finalQuiescencePath=Join-Path $testRoot "phase7b-wp2b-quiescence-$('f'*32).json"
  Write-JsonNoBom $finalQuiescencePath ([ordered]@{classification='PHASE7B_WP2B_NARROW_QUIESCENCE_PASS';pass=$true;toolingCommit=$finalTooling;nonce=('f'*32);observedAt=[DateTime]::UtcNow.ToString('o');monitorTaskDefinitionExact=$true;monitorTaskDisabled=$true;monitorTaskNotRunning=$true;productionServerLeftRunning=$true;productionListenerPresent=$true;autonomousCanonicalWriterPaused=$true;fullCutoverFenceStarted=$false;automaticRetryAllowed=$false})
  $finalQuiescence=Get-Phase7BSha256 -LiteralPath $finalQuiescencePath
  $finalLocalSha=Get-Phase7BSha256 -Text ([IO.Path]::GetFullPath($localOutput).TrimEnd('\').ToLowerInvariant());$finalReplicaSha='c'*64
  $finalAuthorization=New-Phase7BWorkPackage2CaptureAuthorizationDocument -AuthorizationId $finalAuthorizationId -AttemptId $attemptId -ToolingCommit $finalTooling -InvocationContractSha256 $finalInvocation -Stage3LauncherSha256 $finalStage3 -CapturePlanSha256 $capturePlanHash -CapturePlanFileName "$attemptId-capture-plan.json" -InventorySha256 $two.inventorySha256 -SelectionSha256 $selectionHash -SelectionFileName "$attemptId-selection.json" -SourceRootSha256 $sourceRootSha -RuntimeRevision 1 -RuntimeSha256 ('d'*64) -AgeExePathSha256 ('e'*64) -AgeExeSha256 $fakeAgeHash -AgeVersion '1.3.1' -AgeKeygenPathSha256 ('8'*64) -AgeKeygenSha256 $fakeAgeKeygenHash -AgeKeygenVersion '1.3.1' -AgeRecipient $ageRecipient -LocalOutputRootSha256 $finalLocalSha -ReplicaRootSha256 $finalReplicaSha -ReplicaUncRoot "\\LAPTOP-4G5UOU2R\P7B$($attemptId.Substring($attemptId.Length-8))`$" -QuiescenceEvidenceSha256 $finalQuiescence -QuiescenceEvidenceFileName "phase7b-wp2b-quiescence-$('f'*32).json" -QuiescenceEvidenceToolingCommit $finalTooling -ConsumptionMarkerFileName "$finalAuthorizationId.used.json" -IssuedAt ([DateTime]::UtcNow.AddMinutes(-1)) -ExpiresAt ([DateTime]::UtcNow.AddHours(1))
  ...
  Write-JsonNoBom $pendingDescriptorPath (New-Phase7BSyntheticPendingDescriptor -Authorization $finalAuthorization -PacketSha256 $packetSha -PacketBytes $packetBytes)
  ...
  Assert-True ($operationalText.Contains("Arguments = '--decrypt -i -") -and -not $operationalText.Contains('PHASE7B_WP2_ENTER_PASSPHRASE_IN_AGE_TTY')) "native identity stdin contract replaces interactive age TTY"
