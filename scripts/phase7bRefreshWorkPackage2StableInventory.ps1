...
try {
  if($ExpectedToolingCommit -notmatch '^[0-9a-f]{40}$' -or $ExpectedQuiescenceEvidenceSha256 -notmatch '^[0-9a-f]{64}$' -or
  ...
  $attempt="phase7b-wp2-$([guid]::NewGuid().ToString('N'))";$selectionPath=Join-Path $OutputDirectory "$attempt-selection.json";$inventoryAuthPath=Join-Path $OutputDirectory "$attempt-inventory-authorization.json";$planPath=Join-Path $OutputDirectory "$attempt-capture-plan.json"
  if(@(@($selectionPath,$inventoryAuthPath,$planPath)|Where-Object{Test-Path $_}).Count -gt 0){throw 'PHASE7B_WP2B_STABLE_REFRESH_OUTPUT_COLLISION'}
  ...
  [ordered]@{classification='PHASE7B_WP2B_POST_QUIESCENCE_STABLE_INVENTORY_PASS';pass=$true;attemptId=$attempt;toolingCommit=$head;runtimeRevision=[int64]$after.runtimeRevision;runtimeSha256=[string]$after.runtimeSha256;selectionFileName=Split-Path -Leaf $selectionPath;selectionSha256=$selectionSha;inventoryAuthorizationFileName=Split-Path -Leaf $inventoryAuthPath;inventoryAuthorizationSha256=$inventoryAuthSha;capturePlanFileName=Split-Path -Leaf $planPath;capturePlanSha256=Get-Phase7BSha256 -LiteralPath $planPath;sourceInventorySha256=[string]$post.inventorySha256;fileCount=[int]$post.fileCount;totalBytes=[int64]$post.totalBytes;sourceStableAcrossRefresh=$true;sourceMutationPerformed=$false;automaticRetryAllowed=$false}|ConvertTo-Json -Depth 5
} catch {
  $safeCode=if($_.Exception.Message -match '^PHASE7B_'){$_.Exception.Message}else{'PHASE7B_WP2B_STABLE_REFRESH_EXCEPTION'}
  [ordered]@{classification='PHASE7B_WP2B_POST_QUIESCENCE_STABLE_INVENTORY_FAIL';pass=$false;safeStage=$stage;safeErrorCode=$safeCode;writeStarted=$writeStarted;sourceMutationPerformed=$false;automaticRetryAllowed=$false}|ConvertTo-Json -Depth 4
  exit 1
}
