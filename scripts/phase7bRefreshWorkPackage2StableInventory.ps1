...
try {
  ...
  [ordered]@{classification='PHASE7B_WP2B_POST_QUIESCENCE_STABLE_INVENTORY_FAIL';pass=$false;safeStage=$stage;safeErrorCode=$safeCode;safeExceptionType=$_.Exception.GetType().Name;safeLine=$_.InvocationInfo.ScriptLineNumber;writeStarted=$writeStarted;sourceMutationPerformed=$false;automaticRetryAllowed=$false;wp2cAuthorized=$false}|ConvertTo-Json -Depth 4
  exit 1
}
