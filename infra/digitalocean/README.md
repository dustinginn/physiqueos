...
Predeployment boot acceptance is performed without changing the live app specification: the exact worker artifact is run briefly against the isolated compatibility tuple, and the exact product standalone runtime is started on container loopback for liveness, readiness, Home, and static-asset smoke checks. The Home route must use the application composition/read-model boundary; legacy JSON fallback is forbidden. Temporary acceptance artifacts are removed on both success and failure.

Narrow App Platform read authorization independently verified the active rollback deployment as `0d27de79-169a-4fda-a16c-ad868d46b7e4`, source `73c612a539ba056e5dd3b0634a80859f83910787`, and spec digest `2ADB019D77AB20F6AD58AFA5DB7174D539EF2D41A8ED0A04CFB6A5E22BD62602`. It has one 512 MiB web, one 512 MiB worker, web-only ingress, the accepted health check, six web and seven worker encrypted-variable names, and all eight accepted alerts. Secret values were neither printed nor retained in documentation.
