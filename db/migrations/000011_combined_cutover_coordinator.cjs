...
CREATE TABLE physiqueos.combined_cutover_coordinator_runs (
  ...
  coordinator_operation_id text NOT NULL UNIQUE CHECK (coordinator_operation_id ~ '^[A-Za-z0-9][A-Za-z0-9:_-]{7,127}$'),
  migration_operation_id text NOT NULL UNIQUE CHECK (migration_operation_id ~ '^[A-Za-z0-9][A-Za-z0-9:_-]{7,127}$'),
  ...
  failure_code text CHECK (failure_code IS NULL OR failure_code ~ '^[A-Z0-9][A-Z0-9_:-]{0,127}$'),
