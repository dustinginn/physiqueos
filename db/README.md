...
Migration `000011_combined_cutover_coordinator.cjs` adds the narrow external A-P coordination row. It stores only run identity, input digest, CAS version, grouped source step/outcome, safe evidence references, bound approval fingerprints, the safe pre-mutation B Runtime Monitor restoration snapshot/digest, and an M-crossed diagnostic. It is not a second authority, routing, worker, transfer, preparation, or rollback state store: all eligibility and completion decisions are re-read from the existing authoritative stores/receipts. The unique migration operation plus row lock/expected-version update prevent incompatible concurrent advancement. No production migration is executed by landing this schema file.
